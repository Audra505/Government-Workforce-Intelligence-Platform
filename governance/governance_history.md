...
## M33 — Capacity & Gap Intelligence

| ID | Date | Subject | Impact |
|---|---|---|---|
| GD-M33-1 | 2026-07-18 | Capacity & Gap Intelligence — Department-Level Readiness, Attrition, and Vacancy Context | Draft 2026-07-18 (pending approval); produced by a dedicated Phase 4 capability alignment audit that re-evaluated the full intended Phase 4 capability set (vacancy risk, workforce readiness, attrition risk, department gap, demand forecasting, candidate matching, compliance intelligence, executive analytics) against spec/01, spec/15, spec/11, the workforce-forecasting/skill-matching/compliance directives, the Prisma schema, and governance history, and recommended combining department-level breakdown into a single milestone rather than a standalone score; closes the FR-411 "Department Readiness" gap GD-M31-1 explicitly deferred (named 4 times in that document) and the "Department Risk" requirement directives/06_attrition_scoring_rules.md's Reporting Rules section already specified; authorizes NO new scoring formula — WorkforceReadinessService.scoreByDepartment() and AttritionRiskService.scoreByDepartment() reuse GD-M31-1 Decision 5 and GD-M32-1 Decision 5 verbatim, re-scoped by department population instead of tenant population; new DepartmentGapService orchestrates both plus a vacancy-context aggregation reusing VacancyRiskService's existing department join (counts and averages only, not a new score); authorizes GET /api/v1/intelligence/department-gap with no query parameters (deliberately — a caller-suppliable department filter would enable a re-identification drill-down attack) returning every department in one response (RBAC: SA + HR Director + Workforce Planner only; Executive User explicitly NOT granted department-level access pending a dedicated future aggregation-guarantee decision; Recruiter/HM/CO forbidden, unchanged); introduces a governed Minimum Headcount Suppression Rule (constant = 5) — departments below threshold return suppressed:true with no score, no factors, and critically no disclosure of the actual headcount, while vacancy context remains visible for suppressed departments since it is position-level, not person-level, data; formally amends GD-M32-1 Decision 11's "prohibited outright, not merely deferred" department-level attrition language, narrowing it specifically to permit department-level attrition under the new suppression rule for the three allowed roles only, while leaving the tenant-wide endpoint's stronger prohibition fully intact; formally un-defers GD-M31-1's "Department readiness breakdown (FR-411)" Not-In-Scope item under the same conditions; enables the previously-disabled Department Gap tab in the /intelligence workspace (GD-M32-1 Decision 16) with a master-detail layout matching the existing Vacancy Risk tab pattern, explicitly prohibiting any department leaderboard, ranking, or sort-by-score; adds INTELLIGENCE_DEPARTMENT_GAP_QUERIED AuditEventType with PII-safe metadata (formulaVersion, departmentCount, suppressedDepartmentCount only — no department-level detail); explicitly keeps this feature workspace-only — the main dashboard is untouched; explicit no-LLM rule restated unchanged from GD-M30-1 Decision 12; no schema changes; no migrations; no BFF routes; explicitly excludes demand forecasting (schema-blocked — needs a stateful Forecast entity per directives/01), candidate matching and skill gap analysis (schema-blocked — Position/Candidate lack required-skills and skills fields), full compliance intelligence (Phase 5 territory per directives/07 — a Policy/Review/Finding/Exception subsystem, not a Phase 4 score), a Compliance Officer slice, Executive User department-level access, department trend/history, and cross-tenant comparison; 17 governance decisions |
...
| ID | Date | Subject | Impact |
|---|---|---|---|
| GD-M34-1 | 2026-07-19 | Executive Intelligence Expansion & Workforce Snapshot Foundation — Four New Aggregate Metrics and an Additive History Table | Draft 2026-07-19 (pending approval); produced by a focused M34 feasibility drilldown (2026-07-19, following the 2026-07-18 Phase 4 capability alignment audit) that evaluated Executive Intelligence Expansion, Workforce Trend Analysis (FR-412), and Demand Forecasting (FR-400) and found the first fully buildable today while the latter two share one missing prerequisite — real historical data — that does not exist anywhere in the schema; combines the one ready capability with the minimal honest infrastructure step toward the two blocked ones, without claiming either blocked capability complete; authorizes ExecutiveMetricsService with four new deterministic formulas — Vacancy Rate % (open vacancies / active positions), Coverage Rate % (employees with active position / active positions), Time To Fill (average days from creation to fill, 365-day trailing window, reusing GD-M32-1's window length), and Hiring Velocity (count of employees hired in a 90-day trailing window) — each with its own confidence rule and a new metric-level explainability shape (value/unit/confidence/detail/windowDays) alongside, not replacing, the existing IntelligenceExplainabilityOutput contract; authorizes GET /api/v1/intelligence/executive-metrics with no query parameters (RBAC: SA + HR Director + Workforce Planner + Executive User — implements the EU "Hiring velocity KPI / Vacancy rate KPI / Coverage percentage" rows GD-M30-1 Decision 11 already staged for M34; Recruiter/HM/CO forbidden); adds INTELLIGENCE_EXECUTIVE_METRICS_QUERIED AuditEventType with formulaVersion-only metadata; places the four metrics as one new compact-tile card in the dashboard's existing Workforce Intelligence section and as a new subsection inside the existing /intelligence Workforce Signals tab (no new tab); separately authorizes a new additive-only `intelligence` Prisma schema and `WorkforceSignalSnapshot` table (tenantId, signalType, scopeType, scopeId, snapshotDate, score, level, confidence, formulaVersion, computedAt) whose sole purpose is to begin accumulating real computed history from all five Phase 4 signals for a future, separately-governed FR-412/FR-400 milestone; snapshot persistence is write-on-query only (no cron/scheduler/worker — none exists in this codebase) triggered from within the existing score()/getByTenant() methods as an internal, response-shape-invisible side effect; governs a mandatory deduplication rule — a DB-level UNIQUE constraint on (tenantId, signalType, scopeType, scopeId, snapshotDate) with an upsert write path — bounding growth to at most one row per signal per scope per tenant per calendar day regardless of request volume; suppressed Department Gap entries produce no snapshot row; snapshot write failure never affects the triggering request's own response; declares an explicit no-new-exposure guarantee (no read endpoint, query API, or export of snapshot data exists in M34 — the table is written to and read by nothing else); explicitly and repeatedly states this decision does not implement Workforce Trend Analysis or Demand Forecasting, does not claim FR-412 or FR-400 complete, and authorizes no trend chart, forecast score, Forecast entity, or approval workflow of any kind; explicitly excludes Certification Compliance Risk, Compliance Officer intelligence, candidate/skill matching, department-level breakdown of the new metrics, and any change to M30–M33's existing formulas, thresholds, or RBAC; no migration created by this decision (schema described only, per the governance-only scope of this task); 21 governance decisions |
