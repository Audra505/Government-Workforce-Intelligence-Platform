// BFF POST handler — record candidate response (ACCEPTED or DECLINED) for a SENT offer.
// Browser endpoint: POST /api/recruiting/offers/:id/record-response
// Backend target:   POST /api/v1/offers/:id/record-response  (200, transitions SENT → ACCEPTED or DECLINED)
//
// Required body: { response: 'ACCEPTED' | 'DECLINED' }
// Backend precondition: offer.status = SENT — enforced at service layer (OFFER_NOT_SENT 422).
//
...
// Reference: apps/api/src/recruiting/offer.controller.ts — POST /offers/:id/record-response (line 465)
// Reference: apps/api/src/recruiting/dto/record-offer-response.dto.ts
...
export async function POST(
  ...
  if (!body.response || !VALID_RESPONSES.has(body.response as string)) {
    return Response.json(
      { success: false, error: { code: 'VALIDATION_ERROR', message: 'response must be ACCEPTED or DECLINED' } } satisfies BffResponse,
      { status: 400 },
    );
  }
  ...
  try {
    nestRes = await fetch(`${apiUrl}/api/v1/offers/${id}/record-response`, {
      method: 'POST',
      headers: {
