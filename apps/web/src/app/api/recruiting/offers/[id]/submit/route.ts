// BFF POST handler — submit a DRAFT offer for approval.
// Browser endpoint: POST /api/recruiting/offers/:id/submit
// Backend target:   POST /api/v1/offers/:id/submit  (200, transitions DRAFT → PENDING_APPROVAL)
//
// No request body — NestJS submit endpoint takes no body.
//
// SEC-003: no tenantId is accepted from or forwarded by this handler.
//
// Reference: governance/GD-M20-1.md — Decision 3, 7 (Offers)
// Reference: apps/api/src/recruiting/offer.controller.ts — POST /offers/:id/submit (line 292)
...
export async function POST(
  ...
  try {
    nestRes = await fetch(`${apiUrl}/api/v1/offers/${id}/submit`, {
      method: 'POST',
