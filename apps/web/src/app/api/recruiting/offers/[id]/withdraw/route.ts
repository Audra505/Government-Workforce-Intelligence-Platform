// BFF POST handler — withdraw any non-terminal offer.
// Browser endpoint: POST /api/recruiting/offers/:id/withdraw
// Backend target:   POST /api/v1/offers/:id/withdraw  (200, transitions any non-terminal → WITHDRAWN)
//
// No request body — NestJS withdraw endpoint takes no body.
// Terminal offers (ACCEPTED, DECLINED, WITHDRAWN) cannot be withdrawn;
//   backend returns 422 OFFER_IN_TERMINAL_STATE.
//
// SEC-003: no tenantId is accepted from or forwarded by this handler.
//
// Reference: governance/GD-M20-1.md — Decision 3, 7 (Offers)
// Reference: apps/api/src/recruiting/offer.controller.ts — POST /offers/:id/withdraw (line 529)
...
export async function POST(
  ...
  try {
    nestRes = await fetch(`${apiUrl}/api/v1/offers/${id}/withdraw`, {
      method: 'POST',
