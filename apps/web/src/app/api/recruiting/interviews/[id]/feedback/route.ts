// BFF POST handler — record feedback for an interview.
// Browser endpoint: POST /api/recruiting/interviews/:id/feedback
// Backend target:   POST /api/v1/interviews/:id/feedback  (200, returns updated interview)
//
// Required body: { feedback: string }  (non-empty, max 5000 characters)
// Backend allows feedback on SCHEDULED and COMPLETED; blocks on CANCELLED/NO_SHOW
//   with 422 INTERVIEW_FEEDBACK_NOT_ALLOWED.
// UI surfaces feedback only for COMPLETED interviews (GD-M20-1 D7).
//
...
// Reference: apps/api/src/recruiting/interview.controller.ts — POST /interviews/:id/feedback (line 353)
// Reference: apps/api/src/recruiting/dto/interview-feedback.dto.ts
...
export async function POST(
  ...
  if (!feedback || typeof feedback !== 'string' || feedback.trim().length === 0) {
    return Response.json(
      { success: false, error: { code: 'VALIDATION_ERROR', message: 'feedback is required' } } satisfies BffResponse,
      { status: 400 },
    );
  }
  if (feedback.length > 5000) {
    return Response.json(
      { success: false, error: { code: 'VALIDATION_ERROR', message: 'feedback must be 5000 characters or fewer' } } satisfies BffResponse,
      { status: 400 },
    );
  }
  ...
  try {
    nestRes = await fetch(`${apiUrl}/api/v1/interviews/${id}/feedback`, {
      method: 'POST',
      headers: {
