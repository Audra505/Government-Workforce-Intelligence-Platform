// BFF POST handler — mark interview as COMPLETED.
// Browser endpoint: POST /api/recruiting/interviews/:id/complete
// Backend target:   POST /api/v1/interviews/:id/complete  (200, returns updated interview)
//
// No request body — NestJS complete endpoint takes no body.
// Terminal state for status transitions; feedback may still be recorded after COMPLETED.
//
...
// Reference: apps/api/src/recruiting/interview.controller.ts — POST /interviews/:id/complete (line 305)
...
export async function POST(
  ...
  try {
    nestRes = await fetch(`${apiUrl}/api/v1/interviews/${id}/complete`, {
      method: 'POST',
