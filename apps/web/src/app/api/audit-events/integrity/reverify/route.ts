// BFF POST handler — proxies POST /api/audit-events/integrity/reverify →
// NestJS POST /api/v1/audit-events/integrity/reverify.
...
// — reverify accepts no tenant payload of any kind; tenant comes only from
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  let apiRes: Response;
  try {
    apiRes = await fetch(`${base}/api/v1/audit-events/integrity/reverify`, {
      method: 'POST',
