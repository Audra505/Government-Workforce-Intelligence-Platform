// BFF Route Handler — Skills Catalog collection.
// GET:  Browser endpoint  GET /api/skills?page&pageSize&category
//       Backend target:   GET /api/v1/skills
//       Returns paginated skill catalog for the current tenant.
//       Used by client components (assign forms) that cannot call serverFetch.
// POST: Browser endpoint  POST /api/skills
//       Backend target:   POST /api/v1/skills
//       Creates a new skill in the tenant catalog. Returns 201 + created skill.
//
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const url = search ? `${base}/api/v1/skills?${search}` : `${base}/api/v1/skills`;
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  let apiRes: Response;
  try {
    apiRes = await fetch(`${base}/api/v1/skills`, {
      method: 'POST',
      headers: {
