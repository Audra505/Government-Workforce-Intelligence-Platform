// Skills catalog list page — Server Component.
// Fetches GET /api/v1/skills with JWT from session cookie.
// canWrite from JWT roles — gates "New Skill" button (GD-M24-1 D3).
...
import { SkillTable } from '@/features/workforce/components/skill-table';
import { SESSION_COOKIE } from '@/lib/auth';
import type { SkillListApiResponse } from '@/features/workforce/types';
...
async function getSkills(searchParams: PageSearchParams): Promise<SkillListApiResponse> {
  const page = getString(searchParams.page) ?? '1';
  const params = new URLSearchParams();
  params.set('page', page);
  params.set('pageSize', '20');
  return serverFetch<SkillListApiResponse>(`/api/v1/skills?${params.toString()}`);
}
...
function buildPageUrl(searchParams: PageSearchParams, targetPage: number): string {
  ...
  return `/workforce/skills?${params.toString()}`;
}
...
export default async function SkillsPage({ searchParams }: Props) {
  ...
  const { skills, total, page: currentPage, pageSize, totalPages } = response.data;
  ...
  return (
    <WorkforceShell activeTab="skills" breadcrumb="Skills" counts={counts}>
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Skills</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Skill catalog available for assignment to employees
          </p>
        </div>
        {canWrite && (
          <Link
            href="/workforce/skills/new"
      ...
      <SkillTable skills={skills} canWrite={canWrite} />
