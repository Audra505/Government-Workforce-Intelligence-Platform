...
// Minimal toast feedback for Recruiting pages.
// GD-M20-1 D16: bottom-right position; success auto-dismisses at 4s; error at 8s with manual dismiss.
// Usage: call useRecruitingToast() inside any Recruiting Client Component,
//   then render <RecruitingToastContainer toasts={toasts} onDismiss={dismissToast} />.
...
export type RecruitingToastItem = {
  id: string;
  type: 'success' | 'error';
  title: string;
  message?: string;
...
export function useRecruitingToast() {
  const [toasts, setToasts] = useState<RecruitingToastItem[]>([]);

  const addToast = useCallback((item: Omit<RecruitingToastItem, 'id'>) => {
...
export function RecruitingToastContainer({
  toasts,
  onDismiss,
}: {
  toasts: RecruitingToastItem[];
  onDismiss: (id: string) => void;
}) {
