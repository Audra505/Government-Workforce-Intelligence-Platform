...
Last Updated: 2026-09-08 (M40 — Hire and Final-Offer Maker-Checker Resolution: GOVERNANCE DECISION COMPLETE. IMPLEMENTATION NOT STARTED; governance/GD-M40-1.md authorizes a one-packet Tier 2 maker-checker model for final-offer approval/issuance and hire execution, a narrow recruiting-scoped connection to the dormant M38 Decision Case and Approval Foundation, removal of System Administrator from routine offer-approval/issuance/hire-execution with no emergency override and no M37 elevation activation, and a mandatory unnumbered M40→M41 UI-alignment gate; no application, schema, seed, migration, test, configuration, dependency, or UI file has been created or modified; current live offer and hire behavior is completely unchanged; see the M40 section below for full detail)
Updated By: Claude Code (drafted the GD-M40-1.md governance decision from a completed M40 implementation-readiness assessment and the project owner's binding design corrections against it; added the GD-M40-1 entry to governance_history.md; added this file's M40 section and updated the header lines below; performed no implementation work of any kind, per explicit task constraint)

Previous Update: 2026-09-07 (M39 — Audit Read, Recovery, and Integrity Foundation: IMPLEMENTATION COMPLETE AND CI-CONFIRMED; implementation commit 10d352df500726fb243f8e3f6e3fcfe6617d2ca7; GitHub Actions CI run 34087291647 concluded success; see the M39 section below for full detail)

## Repository Status

Current Phase: **Phase 4 — M34 CI-CONFIRMED and CLOSED; post-M34 dashboard analytics enhancement LOCALLY IMPLEMENTED, RUNTIME-VERIFIED, BROWSER-VERIFIED (not yet pushed); M35 Human-in-the-Loop Command Center Blueprint COMPLETE (governance/documentation only); M36 Capability-Based Authorization Foundation IMPLEMENTATION COMPLETE AND CI-CONFIRMED (c4d44f98f75137ae1d18b20ef2c7a2e20e983be2; CI run 33136786479 success) — capability catalog and endpoint-parity safeguards exist and are validated; M37 Elevation Sessions Foundation IMPLEMENTATION COMPLETE AND CI-CONFIRMED (0e3429611a648ce7d0c11fc416187aa21df58490; CI run 33995166375 success) — an internal, dormant elevation-session data model, lifecycle service, and strict-audit-write path now exist and are validated, but remain intentionally inactive; M38 Decision Case and Approval Foundation IMPLEMENTATION COMPLETE AND CI-CONFIRMED (9a6d2653d9108a3e44e1b5a59121b695e950c98a; CI run 34003097416 success) — an internal, dormant Decision Case/Approval data model, three-tier approval engine, and additive M37 scope extension now exist and are validated, but remain intentionally inactive and unconnected to any live workflow or interface; M39 Audit Read, Recovery, and Integrity Foundation IMPLEMENTATION COMPLETE AND CI-CONFIRMED (10d352df500726fb243f8e3f6e3fcfe6617d2ca7; CI run 34087291647 success) — a tenant-scoped, capability-gated audit-read API/viewer, fixed seven-year retention, bounded database-trigger append-only protection, a deterministic per-tenant hash-chain design, automatic failed-write recovery and chain-verification workers, and a coarse, non-gating audit-integrity health signal now exist and are validated, satisfying M39's repository-level contribution to the GD-M35-1 Decision 9 audit prerequisite; RolesGuard remains the sole runtime authorization authority; CapabilityGuard remains unregistered; no capability enforcement is active; no elevation or approval record has any runtime effect; the seven-role user experience is unchanged; Independent Oversight Reviewer activation and authorization-effective elevation each still require their own separate future governance and activation decision; M40 Hire and Final-Offer Maker-Checker Resolution — GOVERNANCE DECISION COMPLETE, IMPLEMENTATION NOT STARTED (governance/GD-M40-1.md authorizes a one-packet Tier 2 maker-checker model, a narrow recruiting-scoped M38 connection, System Administrator exclusion from routine offer/hire authorization with elevation deferred, and the unnumbered M40→M41 UI-alignment gate; current live offer/hire behavior is completely unchanged); M41 and all later milestones remain Planned and unauthorized**
Overall Classification: Phase 2 COMPLETE; Post-Phase-2 milestones M13/M14/M15 CI-confirmed; Pre-Phase-3 Governance Package CI-confirmed (a5c34f1); Phase 3 started — M16 CI-confirmed; M17 CI-confirmed; M18 CI-confirmed; M19 CI-confirmed; M20 CI-confirmed (6e6777b; run 28611838113); M21 CI-confirmed (1036c92 + 3c8189d + 1e33420); browser-verified by human 2026-07-03; CLOSED; M21.5 CI-confirmed (782e35e + 1a4b64f; runs #66 + #67); M22 CI-confirmed (ee8465b); browser-verified by human 2026-07-04; CLOSED; M23 CI-confirmed (5fedb81); browser-verified by human 2026-07-06; CLOSED; M24 CI-confirmed (5f5bfa6); browser-verified by human 2026-07-11; CLOSED; M25 CI-confirmed (23d46ef); browser-verified by human 2026-07-13; CLOSED; M26 CI-confirmed (a9a6943); runtime-verified 2026-07-14; CLOSED; M27 CI-confirmed (9049fd6); runtime-verified 2026-07-14; CLOSED; M28 CI-confirmed (46ffcce); runtime-verified + human browser-verified 2026-07-14; CLOSED; M29 CI-confirmed (45efe2f); seed validated 2026-07-14; CLOSED; Phase 3 COMPLETE; Phase 4 Intelligence STARTED — M30 CI-confirmed (407195b); governance GD-M30-1.md authorized (3c9366e) and amended twice (d3a2a65, 0401672); runtime-verified + browser-verified 2026-07-16; CLOSED; M31 CI-confirmed (cbcacdb); governance GD-M31-1.md authorized (051fa8f); runtime-verified + browser-verified 2026-07-16; CI run #99 (ID 29534898812) completed / success; CLOSED; M32 CI-CONFIRMED (dad0afc; run 29654755142 completed/success); governance GD-M32-1.md authorized (e43c700) and amended with Amendment 1 (Intelligence Detail Workspace); runtime-verified + browser-verified 2026-07-18; CLOSED; M33 CI-CONFIRMED (466350f; run 29658955778 completed/success); governance GD-M33-1.md authorized (2026-07-18, pending approval, local); department-level Workforce Readiness/Attrition Risk (scoreByDepartment()) + DepartmentGapService + GET /api/v1/intelligence/department-gap + Department Gap tab; runtime-verified + browser-verified 2026-07-18; CLOSED; M34 CI-CONFIRMED (0d0ef60; run 29803545493 completed/success); governance GD-M34-1.md authorized (2026-07-19, pending approval, local); ExecutiveMetricsService (Vacancy Rate %, Coverage Rate %, Time To Fill, Hiring Velocity) + GET /api/v1/intelligence/executive-metrics + additive-only WorkforceSignalSnapshot table with DB-enforced dedup/upsert (no read endpoint) + purpose-built Executive User dashboard + platform-wide role navigation cleanup + shared PlatformHeader consolidation + a login/logout client-router-cache role-switch bug found and fixed; CLOSED; Post-M34 (not a numbered milestone) — Operational Snapshot dashboard cards enhanced with lightweight, real-data-only analytics (30-day hires/separations/net change, coverage-rate context, 30-day opened/filled/net change, oldest-critical-vacancy age and share) reusing existing RBAC boundaries and existing/extended `GET /employees` and `GET /vacancies` query filters; role consistency re-verified across all 7 fixture roles with no mismatches; LOCALLY IMPLEMENTED, RUNTIME-VERIFIED, BROWSER-VERIFIED; staged for one local commit, not pushed; OPEN pending push/CI; M35 (Human-in-the-Loop Command Center Blueprint) — GOVERNANCE/DOCUMENTATION COMPLETE (governance/GD-M35-1.md + governance/M35-COMMAND-CENTER-BLUEPRINT.md; governance_history.md indexed); no code, schema, migration, seed data, test, UI, or runtime AI change of any kind; no capability implemented, no capability validated; the target platform redesign remains Planned maturity; CLOSED as a blueprint/governance milestone; M36 (Capability-Based Authorization Foundation) — IMPLEMENTATION COMPLETE AND CI-CONFIRMED (governance/GD-M36-1.md's Implementation Completion Record; governance_history.md's GD-M36-1 entry updated in place); implementation commit c4d44f98f75137ae1d18b20ef2c7a2e20e983be2 seeds the dormant Permission/RolePermission tables with the 73-endpoint/62-unique-permission/175-RolePermission-mapping capability catalog, adds a PermissionsService, method-level @RequireCapability metadata on all 73 protected handlers, and a non-registered CapabilityGuard, proven by 9,271/9,271 deterministic parity comparisons; GitHub Actions CI run 33136786479 (exact head_sha match) concluded success across Lint/Build/Unit/Migrate/Seed/E2E; RolesGuard remains the sole runtime authorization authority; no capability enforcement is activated; no currently authorized or forbidden endpoint outcome changed; all 70 pre-existing @RequireRoles(...) occurrences, all seven legacy roles, and all existing fixture identities are unchanged; JWT/session contract and frontend are unchanged; CLOSED; M37 (Elevation Sessions Foundation) — IMPLEMENTATION COMPLETE AND CI-CONFIRMED (governance/GD-M37-1.md's Implementation Completion Record; governance_history.md's GD-M37-1 entry updated in place); implementation commit 0e3429611a648ce7d0c11fc416187aa21df58490 adds an internal, dormant, tenant-scoped ElevationSession/ElevationSessionCapability data model and lifecycle (no HTTP controller, route, API, frontend interface, feature flag, CapabilityGuard registration, RolesGuard change, or JWT claim of any kind), a 4-capability grantable allowlist (users:create, users:read, users:update, roles:assignable:read) copied verbatim from the live, independently re-verified M36 catalog — three of the four operate on sensitive User account identity data (name, email, role assignment) while none exposes Employee or Candidate PII — requester may equal grantee (self-requested elevation permitted) but the approver must always differ from both with no self-approval exception in M37 (actor tenant membership validated at the service layer since actor columns carry no FK), while the revoker need not be distinct from the other actors, a narrow strict-audit-write exception scoped to elevation transitions only (does not alter AuditService.logEvent()'s existing behavior or satisfy any part of M39), and nullable step-up-evidence fields that remain null throughout M37 (no genuine MFA/step-up mechanism exists in this repository); GitHub Actions CI run 33995166375 (exact head_sha match) concluded success across Lint/Build/Unit/Migrate/Seed/E2E; 101/101 M37-targeted tests, 60 suites/11,494 unit tests, and 14 suites/750 E2E tests passed; RolesGuard remains the sole runtime authorization authority; no elevation record has any runtime effect; M39 activation prerequisite remains unsatisfied; CLOSED; M38 (Decision Case and Approval Foundation) — IMPLEMENTATION COMPLETE AND CI-CONFIRMED (governance/GD-M38-1.md's Implementation Completion Record; governance_history.md's GD-M38-1 entry updated in place); implementation commit 9a6d2653d9108a3e44e1b5a59121b695e950c98a adds an internal, dormant Decision Case and Approval foundation (no HTTP controller, route, public API, frontend interface, Command Center UI, change to any existing business endpoint or its authorization, business-action execution, runtime-effective approval, runtime-effective elevation, new production authority profile, CapabilityGuard activation, JWT change, or AI/LLM/autonomous behavior of any kind) comprising DecisionCase, EvidenceItem, DecisionCaseRiskTrigger, ApprovalRequest, ApprovalRequirement, and ApprovalDecision (append-only, immutable, no PENDING value) and ApprovalRequestEvidence in a new "decisions" Postgres schema, all three approval tiers implemented as internal data/service rules only with routine hires/offers never requiring Tier 3 by default and bidirectional Tier 3 independence proven against a real database, temporary legacy-role-to-authority-category qualification mappings that do not activate the future Workforce Decision Owner or Independent Oversight Reviewer profiles and confirm System Administrator does not auto-qualify, and an additive, authorization-inert extension of ElevationSession (decisionCaseId, governed scopeType, real restrictive scopeDepartmentId/scopeDecisionCaseId foreign keys, isElevationLifecycleCapabilityValidAt() unchanged); PolicyEvaluation remains deferred to M42, execution/verification entities to a later milestone, and AIRecommendation/AgentRun to M44/M45 — none exist anywhere in this implementation; GitHub Actions CI run 34003097416 (exact head_sha match) concluded success across Lint/Build/Unit/Migrate/Seed/E2E; 124 M38-targeted tests, 63 suites/11,612 unit tests, and 15 suites/773 E2E tests passed; RolesGuard remains the sole runtime authorization authority; no capability enforcement is activated; no decision-case or approval record has any runtime effect; the hire-endpoint RBAC inconsistency remains unresolved and unchanged, still assigned exclusively to M40; M39's audit-read/viewer/tamper-detection/retention prerequisites were unimplemented at the time M38 closed and have since been implemented — see the M39 entry immediately below; CLOSED; M39 (Audit Read, Recovery, and Integrity Foundation) — IMPLEMENTATION COMPLETE AND CI-CONFIRMED (governance/GD-M39-1.md's Implementation Completion Record; governance_history.md's GD-M39-1 entry updated in place); implementation commit 10d352df500726fb243f8e3f6e3fcfe6617d2ca7 implements the first HTTP surface since M36 — exactly five endpoints (audit-event list/detail, recovery-status, requeue, reverify), three gated by a new audit:read capability (System Administrator, Compliance Officer) and two narrowly-scoped mutations gated by a new audit:recover capability (System Administrator only) — projecting the M36 baseline to 64 capabilities/178 role-permission mappings/78 protected endpoints/78 capability mappings/73 executable @RequireRoles() occurrences/127 role subsets/9,906 parity comparisons/9 canonical role-set combinations, with all 73 pre-existing mappings unchanged; implements a fixed, non-configurable seven-year retention floor (no deletion, no archival movement, no purge, no RetentionPolicy table), a database-trigger-enforced append-only control explicitly bounded (never absolute immutability, never protection against the database owner/superuser, and explicitly excluding database-role separation from M39 scope, both re-confirmed live via direct UPDATE/DELETE rejection against a real database), a fully deterministic per-tenant SHA-256 hash-chain tamper-evidence design (proven against a real database, including a live tamper-and-detect run), three additive tables (AuditWriteFailure, AuditWriteFailureAttempt, AuditChainState) with a complete immutable per-attempt recovery history, automatic database-coordinated recovery and chain-verification workers (@nestjs/schedule, one new dependency, real-timer-fire proven in non-test runtime), a coarse, non-gating audit-integrity health signal on /health (project-owner clarification recorded under GD-M39-1 Decision 21 — a broken chain or recovery backlog reports auditIntegrity: "degraded" without ever failing the endpoint, since restarting cannot repair either condition), direct real-transaction failure-injection proof that both requeue and reverify roll back completely on a forced strict-audit-write failure, and six new AuditEventType values (122→128, 128 total); GitHub Actions CI run 34087291647 (exact head_sha match) concluded success across Lint/Build/Unit/Migrate/Seed/E2E; confirms RolesGuard remains sole runtime authority and CapabilityGuard remains unregistered; M37 ElevationSession remains entirely authorization-inert and M38 DecisionCase/ApprovalRequest/ApprovalDecision records remain entirely disconnected from any live workflow, neither modified by this implementation; frontend validation covered type-check/lint/build/route-generation/API-level E2E/code-review — no interactive browser walkthrough was performed, a disclosed non-blocking limitation; this implementation is M39's repository-level contribution to the GD-M35-1 Decision 9 audit prerequisite — Independent Oversight Reviewer activation and authorization-effective elevation each still require their own separate future governance and activation decision; CLOSED; M40 (Hire and Final-Offer Maker-Checker Resolution) — GOVERNANCE DECISION COMPLETE, IMPLEMENTATION NOT STARTED (governance/GD-M40-1.md; governance_history.md's GD-M40-1 entry added); authorizes a one-packet Tier 2 maker-checker model for final-offer approval/issuance and hire execution (one DEPARTMENT_OR_HIRING_AUTHORITY + one HR_AUTHORITY, two distinct humans, reusing the existing dormant M38 ApprovalService/authority-category-mapping.ts unmodified), a narrow recruiting-scoped connection to the dormant M38 Decision Case and Approval Foundation (offer-subject DecisionCase, no generic Decision Case API), a fully resolved, binding Offer-status transition table (no new status, no migration; REJECTED maps to the existing terminal WITHDRAWN with documented semantic justification, NEEDS_INFORMATION maps to the existing editable DRAFT), removal of System Administrator from offers:submit/offers:approve/offers:issue/hire:authorize (four capabilities) with no emergency override and no M37 elevation activation, retirement of POST /offers/:id/approve (replaced by three new recruiting-scoped routes, net −1+3), one new narrowly scoped read capability (offers:approvals-read, excluding both System Administrator and Compliance Officer) rather than reusing the broader offers:read, no new Prisma migration, exactly one new (projected) AuditEventType value classified as operational (not strict), a minimum recruiting-scoped approval UI reusing the existing shell, and a mandatory unnumbered M40→M41 UI-alignment gate referencing the project owner's newer BI Consulting platform without renumbering M41–M46; explicitly supersedes only the hire-execution portion of GD-M19-1 Decision 3; no application, schema, seed, migration, test, configuration, dependency, or UI file was created or modified; current live offer and hire behavior is completely unchanged; RolesGuard remains the sole runtime authorization authority; CapabilityGuard remains unregistered; M37 elevation remains authorization-inert; Independent Oversight Reviewer remains inactive; OPEN pending a separate, explicit implementation instruction
Active Sprint / Milestone: M38 — Decision Case and Approval Foundation is IMPLEMENTATION COMPLETE AND CI-CONFIRMED and CLOSED (implementation commit 9a6d2653d9108a3e44e1b5a59121b695e950c98a; CI run 34003097416); M39 — Audit Read, Recovery, and Integrity Foundation is IMPLEMENTATION COMPLETE AND CI-CONFIRMED and CLOSED (implementation commit 10d352df500726fb243f8e3f6e3fcfe6617d2ca7; CI run 34087291647); M39's repository-level contribution to the GD-M35-1 Decision 9 audit prerequisite is implemented and CI-confirmed; M40 — Hire and Final-Offer Maker-Checker Resolution is GOVERNANCE DECISION COMPLETE (governance/GD-M40-1.md, 2026-09-08) and IMPLEMENTATION NOT STARTED — current live offer and hire behavior is unchanged; the next action is a separate, explicit implementation instruction operating within GD-M40-1's authorized scope; Independent Oversight Reviewer activation, authorization-effective elevation, role cutover, CapabilityGuard activation, and any later operating-model capability each still require their own separate, future governance and activation decision; the pre-existing post-M34 dashboard analytics enhancement remains staged for one local commit, not pushed, unaffected by M35 through M40
...
CI impact: governance commits touch only `governance/` and `PROGRESS.md`. No source code, no schema, no migrations. CI / Install, Lint, Build, Test — **CONFIRMED GREEN** (2026-06-27; commit a5c34f1; run 28280426818; success).
...
4. ✅ M17 Application Management Foundation implemented (Steps 1–5 committed; 8 commits ahead of origin/main)
...
| (3 earlier M17 commits) | Steps 1–3: migration, ApplicationService foundation, ApplicationModule wiring |
...
| POST create (shape, SEC-003, VAC-301) | 11 | ✅ PASS |
| Validation DTOs | 6 | ✅ PASS |
| Duplicate prevention → 409 | 4 | ✅ PASS |
| GET list (pagination, filters) | 13 | ✅ PASS |
| GET detail (shape, tenantId excluded) | 9 | ✅ PASS |
| PUT update (notes, stage) | 9 | ✅ PASS |
| Advance state machine (APPLIED→SCREENING→INTERVIEW→EVALUATION→OFFER; AWAITING_HIRE guard) | 9 | ✅ PASS |
| Reject → 200 | 6 | ✅ PASS |
| Withdraw → 200 | 6 | ✅ PASS |
| Re-application after REJECTED or WITHDRAWN | 3 | ✅ PASS |
| VAC-301 (OPEN→IN_RECRUITMENT on first application) | 3 | ✅ PASS |
| Archive guard → 422 CANDIDATE_HAS_ACTIVE_APPLICATIONS | 5 | ✅ PASS |
| Tenant isolation (cross-tenant → 404) | 6 | ✅ PASS |
| Audit events (5 event types) | 8 | ✅ PASS |
| **TOTAL** | **101** | **✅ 101/101 PASS** |

#### Controller Unit Specs — PASS (107/107)

- `application.controller.spec.ts` — all endpoints × all outcome codes
- `candidate.controller.spec.ts` — including updated CANDIDATE_HAS_ACTIVE_APPLICATIONS → 422 UnprocessableEntityException

#### API Build — PASS

`npm run build --workspace=apps/api` → nest build EXIT 0; `dist/main.js` compiled 2026-06-29 19:33.

Compiled `dist/main.js` startup logs confirm ApplicationController registered with all 7 routes.

#### Lint — PASS

`npm run lint --workspace=apps/api` → EXIT 0, no ESLint warnings or errors.
...
| `apps/api/src/recruiting/candidate.controller.ts` | `CANDIDATE_HAS_ACTIVE_APPLICATIONS` → `UnprocessableEntityException` (422); was `ConflictException` (409) | M16 stub was placeholder; M17 activates live guard; 422 is correct per GD-M17-1 business-rule pattern |
...
| State Model | Implemented (APPLIED→SCREENING→INTERVIEW→EVALUATION→OFFER; terminal states HIRED/REJECTED/WITHDRAWN) |
...
# M36 — Capability-Based Authorization Foundation (Implementation Complete, CI-Confirmed)

> **Repository Status Classification: IMPLEMENTATION COMPLETE. CI-CONFIRMED. CLOSED.**
> This entry records both the completion of `GD-M36-1` (the governance decision) and the
> completion of the M36 implementation it authorized. Capability-catalog foundations and
> endpoint-parity safeguards now exist and are validated. `CapabilityGuard` is implemented and
> unit-tested but is **not** wired into any runtime enforcement path — `RolesGuard` remains the
> sole runtime authorization authority, no currently authorized or forbidden endpoint outcome
> changed, and the existing seven-role user experience is unchanged. This entry must not be read,
> cited, or summarized as evidence that capability-based enforcement is active, that any role was
> retired, that a two-profile cutover occurred, or that M37 has been authorized or started.

**Date:** 2026-08-27 (governance decision, `GD-M36-1`); 2026-08-28 (implementation completed and
CI-confirmed)

**Status:** IMPLEMENTATION COMPLETE AND CI-CONFIRMED.
Implementation commit `c4d44f98f75137ae1d18b20ef2c7a2e20e983be2`
(`feat(auth): implement M36 capability foundation`). GitHub Actions CI run `33136786479`
(workflow `CI`, exact `head_sha` match) concluded **success** — Lint, Build, Unit tests, Migrate
database, Seed database, and E2E tests all passed. `RolesGuard` remains the sole runtime
authorization authority; no capability enforcement of any kind is active.

## Capability / Deliverable Alignment

- **Capability:** Capability-Based Authorization Foundation
...
- **Current maturity classification:** **Implemented / Integrated / Tested** (per CLAUDE.md's
  Repository Maturity Classification) for the foundation layer specifically — the capability
  catalog, seed extension, resolver, decorator, guard, and parity-proof test suite exist, are
  unit- and CI-tested, and are proven equivalent to current `RolesGuard` behavior across every
  endpoint and role combination. **Not** "Production Ready" for capability *enforcement* — that
  remains explicitly unactivated and unauthorized (Decision 8, Decision 19 → M46)
- **Relevant production blueprint layers covered:** Requirements and Directives (`GD-M36-1.md`,
  including Appendix A); Execution Plan (implemented exactly as authorized); State Model
  (`PermissionsService`, `@RequireCapability`, `CapabilityGuard` all exist and are unit-tested);
  Test Scenarios (the deterministic parity-proof suite, 9,271/9,271 comparisons, plus edge-case
  and drift-protection coverage); System Loop (the seed step runs as part of the existing
  deterministic seed script, itself CI-verified)
- **Relevant production blueprint layers still missing or incomplete (by design — deferred to a
  future, separately governed milestone):** Failure Playbook and Environment Model for
  *enforcement* (fail-closed behavior is implemented and unit-proven for `CapabilityGuard` in
  isolation, but has no runtime environment to operate in yet, since the guard is never
  registered); Data Lifecycle for capability grants beyond the static seed (no elevation,
  expiration, or revocation model exists — that is M37/M38's scope); Evolution Strategy
  (M37/M38/M40/M46 roadmap ownership remains documentation-only, per Decision 19 — none of those
  milestones is authorized or started by this entry)

## What Changed

**Files created (governance decision, 2026-08-27):**
- `governance/GD-M36-1.md` — see prior entry text (unchanged); the governance decision and its
  full Appendix A catalog remain exactly as originally authorized.

**Files modified (governance decision, 2026-08-27):**
- `governance/governance_history.md`, `PROGRESS.md` header — governance-decision completion
  recorded (superseded by this entry's implementation-completion record below).

**Files created (implementation, commit `c4d44f9`, 2026-08-28):**
- `apps/api/src/identity/permissions.catalog.ts` — single source-of-truth capability catalog: 62
  unique capability keys, 7 legacy role-set constants, the 62-entry `CAPABILITY_ROLE_MAPPINGS`
  table (175 total role-permission pairs), 4 reserved-but-unseeded capabilities, and the
  `resource:action` split/join helpers
- `apps/api/src/identity/permissions.seed.ts` — idempotent `seedPermissionsAndRolePermissions()`,
  upsert-only against the schema's natural unique constraints, shared by both the real seed
  script and its own unit test
- `apps/api/src/identity/permissions.service.ts` — `PermissionsService.resolve()`: union-of-roles
  capability resolution; propagates database/resolver failures rather than swallowing them
- `apps/api/src/identity/decorators/require-capability.decorator.ts` — `@RequireCapability(...)`,
  structurally identical to the existing `@RequireRoles(...)` decorator
- `apps/api/src/identity/capability.guard.ts` — `CapabilityGuard`: fail-closed on resolver
  failure or invalid resolver result; implemented and unit-tested; **not registered on any
  controller, global guard list, or application bootstrap path**
- Four new spec files: `permissions.catalog`/`permissions.seed`/`permissions.service`/
  `capability.guard` unit tests, plus `capability-parity.spec.ts` — the deterministic parity-proof
  suite (9,373 total tests: 9,271 endpoint/role-subset comparisons plus 102 supporting invariant
  and drift-protection assertions)

**Files modified (implementation, commit `c4d44f9`, 2026-08-28):**
- `apps/api/prisma/seed.ts` — additive-only call to `seedPermissionsAndRolePermissions()`,
  runs in every environment (not gated by `NODE_ENV`); never deletes, truncates, or replaces any
  existing `Role`, `User`, or `UserRole` row
- `apps/api/src/identity/identity.module.ts` — registers `PermissionsService` and
  `CapabilityGuard` as injectable providers for dependency injection and future use only; neither
  is added to any `@UseGuards(...)` list or `APP_GUARD`
- 15 controller files — each edit is limited to two import lines plus one inert
  `@RequireCapability(CAPABILITIES.X)` decorator per protected handler (73 handlers total across
  all 15 files); every one of the 70 pre-existing `@RequireRoles(...)` decorator occurrences is
  byte-for-byte unchanged; zero routes, DTOs, service calls, response shapes, or business logic
  were touched (confirmed by diff inspection: every added controller line is an import or a
  `@RequireCapability(...)` line, zero lines were removed)

**Architectural changes:** `Permission`/`RolePermission` tables (schema-present since an earlier
milestone, previously fully dormant) are now seeded. No Prisma schema or migration file was
created or modified — the existing schema already supported this catalog. No new module,
endpoint, or route was created.

**New integrations:** None. `PermissionsService` and `CapabilityGuard` are injectable but consumed
by nothing at runtime.

**Behavioral changes:** None observable to any caller. Every existing endpoint's currently
authorized and currently forbidden role set is unchanged — proven by the parity-proof suite
comparing real `@RequireRoles(...)` metadata against the new catalog across all 73 endpoints and
all 127 non-empty role subsets (9,271 comparisons, zero mismatches). `RolesGuard`,
`require-roles.decorator.ts`, `jwt.strategy.ts`, `auth.service.ts`, and every frontend
role-consuming file remain byte-for-byte unchanged — no frontend file was touched by this
implementation.

## Validation

- **Tests added:** `permissions.catalog`/`permissions.seed`/`permissions.service`/
  `capability.guard` unit specs (33 tests) plus `capability-parity.spec.ts` (9,373 tests,
  including the 9,271 endpoint/role-subset parity comparisons, the 7-role-combination check, the
  11-shared-capability check, reserved-capability-absence checks, Executive User PII-exclusion
  checks, Compliance Officer read-only checks, and the permanent metadata-drift guard). No
  existing test file was weakened, removed, or skipped.
- **Local validation results:** type-check clean (0 errors); lint clean (0 errors/warnings, all
  5 workspaces); build clean (exit 0, all workspaces); unit tests 58/58 suites, 11,403/11,403
  tests passed (confirmed in 3 separate isolated runs); E2E tests 13/13 suites, 740/740 tests
  passed; Prisma schema validated (`npx prisma validate` — valid, no changes); seed idempotency
  proven against the local development database — first run upserted exactly 62 Permission rows
  and 175 RolePermission mappings from an empty baseline, second run reported the identical
  counts and left the database in an identical state (zero duplicates, zero drift); all 7
  platform roles and existing fixture identities confirmed present and unchanged before and after
  seeding.
- **CI validation results:** GitHub Actions run `33136786479` (workflow `CI`, exact `head_sha`
  match to the implementation commit) — all 15 steps succeeded, including Lint, Build, Unit
  tests, Migrate database, Seed database, and E2E tests.
- **Deterministic validation evidence:** The parity-proof suite is itself the deterministic
  evidence — it reads real `@RequireRoles(...)` and `@RequireCapability(...)` metadata directly
  off the 15 controller classes (no hand-maintained duplicate endpoint list) and asserts
  equivalence against `CAPABILITY_ROLE_MAPPINGS` for every endpoint and every non-empty subset of
  the seven platform roles.
- **Integration validation status:** Full integration validated through the real E2E suite
  (in-process NestJS app + real Postgres, no mocking) and the live local seed run against the
  actual development database — not merely unit-level.
- **Known unverified areas:** None within M36's authorized scope. The candidate audit item noted
  in `GD-M36-1.md` Decision 12 (whether `applications:advance` exhibits the same
  one-route/multiple-semantic-outcomes pattern as the employee-status and user-update endpoints)
  remains an explicitly flagged, unresolved item for a future milestone's audit — not resolved by
  M36, and not required to be.

## Risks / Limitations

- **Capability enforcement is not active.** `CapabilityGuard` exists, is unit-tested, and is
  proven equivalent to `RolesGuard` in every case the parity suite covers, but it is not attached
  to any route. Activating it is explicitly reserved for a future, separately governed milestone
  (`GD-M36-1` Decision 8, Decision 19 → M46 by default) — this entry makes no claim that
  enforcement will activate automatically or without its own governance decision.
- **The seven legacy roles are unchanged and un-retired.** No role was removed, renamed, or
  restricted. The current seven-role user experience — for every one of System Administrator, HR
  Director, Workforce Planner, Recruiter, Hiring Manager, Compliance Officer, and Executive
  User — is identical before and after this implementation.
- **The hire-endpoint RBAC inconsistency** (`hire.controller.ts:57` vs. `directives/10` RBAC-701)
  remains unresolved in the running system, unchanged by M36, and explicitly assigned to M40 —
  `hire:authorize` is seeded mapped to today's exact SA/HRD behavior, unchanged.
- **The four reserved conditional capabilities** (`employees:terminate`, `users:deactivate`,
  `users:assign-role`, `offers:rescind`) remain documented but unseeded and unimplemented — no
  route carries their metadata, and no future milestone may assume they exist without its own
  seed/catalog change.
- **The scoped-grant model** (tenant/department/expiration/second-approval/self-approval) required
  by the eventual two-profile cutover does not exist. M37 and M38 jointly own building it; neither
  has been authorized or started by this entry.
- **JWT/session behavior is unchanged**, including the existing one-hour token-staleness window
  documented in `GD-M36-1` Decision 18 as a known pre-existing limitation, not a property this
  milestone introduced or endorses as target design.
- **Pre-existing formatting debt.** 15 of the 16 modified controller/seed files were already
  non-compliant with the repository's Prettier configuration before M36 touched them (verified by
  checking each file's pre-M36 HEAD version independently) — mostly long single-line
  `@ApiResponse(...)` decorators and import statements exceeding the configured line width. M36
  left this pre-existing debt untouched rather than reformatting unrelated code inside files it
  was already editing; the one file where M36's own edit introduced a genuine new formatting
  violation (`identity.module.ts`) was corrected before commit.
- **A non-M36 local timing observation, not a validation failure.** During local validation, one
  test in `intelligence.controller.spec.ts` exceeded jest's default 5-second timeout a single time
  when the full local suite ran under heavy concurrent CPU load (alongside the newly-added
  9,271-comparison parity suite in the same process). The same test passed reliably across three
  separate clean isolated re-runs (worst case 890ms, well under the limit), and the exact-SHA
  implementation CI run above completed every test step, including this one, with no timeout.
  This is recorded as a local-environment observation; it does not indicate an M36 defect and does
  not weaken the "all local validation passed" conclusion above.

## Next Actions

1. M36 is closed. No further M36 implementation work is authorized or required.
2. **M37 — Elevation Sessions** is the next milestone in the `GD-M35-1`/`GD-M36-1` roadmap. It
   remains **Planned** maturity and is **not authorized to begin** — it requires its own separate,
   explicit governance decision (following the `GD-M36-1` pattern: readiness assessment →
   governance decision → separate implementation instruction) before any code, schema, or test
   file for it may be created. Per `GD-M36-1` Decision 19, M37 owns building the reusable
   elevation/scoped-grant core (tenant, grantee, capability, purpose, start, expiration,
   revocation state) — building the foundation, not activating production elevation.
...
# M37 — Elevation Sessions Foundation (Implementation Complete, CI-Confirmed)

> **Repository Status Classification: IMPLEMENTATION COMPLETE. CI-CONFIRMED. CLOSED.**
> This entry records the completion of `GD-M37-1` (the governance decision) and the completion of
> the M37 implementation it authorized. The elevation-session foundation now exists and is
> validated, but it is **internal, dormant, and intentionally inactive** — `RolesGuard` remains the
> sole runtime authorization authority, `CapabilityGuard` remains unregistered, no endpoint or UI
> exists for elevation, no genuine step-up/MFA mechanism was introduced, and no currently
> authorized or forbidden endpoint outcome changed. This entry must not be read, cited, or
> summarized as evidence that elevated access is active, that M39 is complete, that a two-profile
> cutover occurred, or that M38 has been authorized or started.

**Date:** 2026-09-05 (governance decision, `GD-M37-1`, and implementation completed and
CI-confirmed the same day)

**Status:** IMPLEMENTATION COMPLETE AND CI-CONFIRMED.
Implementation commit `0e3429611a648ce7d0c11fc416187aa21df58490`
(`feat(auth): implement M37 elevation sessions foundation`). GitHub Actions CI run `33995166375`
(workflow `CI`, exact `head_sha` match) concluded **success** — Lint, Build, Unit tests, Migrate
database, Seed database, and E2E tests all passed. `RolesGuard` remains the sole runtime
authorization authority; no elevation record has, or can have, any effect on a real authorization
decision.

## Capability / Deliverable Alignment

- **Capability:** Elevation Sessions Foundation (internal, dormant)
- **Deliverable relevance:** Second implementation milestone in the `GD-M35-1` post-M35 roadmap;
  builds the reusable elevation/scoped-grant core `GD-M36-1` Decision 19 already named
  (documentation only) as M37's job — the foundation a future, separately governed activation
  milestone would need before any temporary System Administrator elevation could replace the
  permanent standing persona
- **Current maturity classification:** **Implemented / Integrated / Tested** (per CLAUDE.md's
  Repository Maturity Classification) for the foundation layer specifically — the normalized data
  model, migration, internal service, allowlist, strict audit-write path, and structural
  non-reachability proof exist, are unit- and CI-tested, and are proven to have zero effect on any
  real authorization decision. **Not** "Production Ready" for elevation *activation* — that remains
  explicitly unactivated and gated on M39 plus a separate future governance decision (`GD-M37-1`
  Decision 21)
- **Relevant production blueprint layers covered:** Requirements and Directives (`GD-M37-1.md`,
  including Appendix A); Execution Plan (implemented exactly as authorized); State Model
  (`ElevationSessionStatus`/`ElevationCapabilityDecision` Prisma enums, `ElevationSessionService`
  all exist and are tested against a real database); Test Scenarios (mocked unit suite, structural
  non-reachability proof, and a real-database integration suite covering strict-audit
  transactionality and concurrency); System Loop (none required — no scheduler, cron, or worker
  exists or is authorized); Data Lifecycle (create → decide → approve/deny → activate/fail →
  revoke/expire, all persisted and tenant-scoped)
- **Relevant production blueprint layers still missing or incomplete (by design — deferred to a
  future, separately governed milestone):** Failure Playbook and Environment Model for
  *activation* (no runtime consumer of `ElevationSession` exists yet — activation requires
  trustworthy step-up authentication, M39 operational, and its own governance decision, per
  Decision 21); Evolution Strategy for department/decision-case/self-approval-exception scope
  (M38's scope, not started by this entry)

## What Changed

**Files created (governance decision, 2026-09-05):**
- `governance/GD-M37-1.md` — see prior entry text (unchanged); the governance decision and its
  full Appendix A allowlist remain exactly as originally authorized. 23 governance decisions.

**Files modified (governance decision, 2026-09-05):**
- `governance/governance_history.md`, `PROGRESS.md` header — governance-decision completion
  recorded (superseded by this entry's implementation-completion record below).

**Files created (implementation, commit `0e34296`, 2026-09-05):**
- `apps/api/prisma/migrations/20260801000000_m37_elevation_sessions/migration.sql` — creates the
  first native Postgres enums in this schema (`ElevationSessionStatus`, 8 values;
  `ElevationCapabilityDecision`, 3 values), the `identity.elevation_sessions` and
  `identity.elevation_session_capabilities` tables, the tenant-scoped idempotency unique index, the
  per-session capability-uniqueness index, the partial unique index enforcing "at most one
  non-terminal session per (tenant, grantee)" (SQL-only, following the `Offer` precedent), and a
  defense-in-depth `CHECK` constraint rejecting `approvedByUserId = requestedByUserId` or
  `= granteeUserId`. No `ON DELETE CASCADE` exists on any actor-reference column — verified
  directly against the applied schema.
- `apps/api/src/identity/elevation-session-allowlist.ts` — the hard-coded 4-capability allowlist
  (`users:create`, `users:read`, `users:update`, `roles:assignable:read`), referencing
  `permissions.catalog.ts`'s live constants directly (no duplicated string literals).
- `apps/api/src/identity/elevation-session.service.ts` — `ElevationSessionService`: no HTTP
  interface; implements `requestElevation`, `decideElevationCapabilities`,
  `cancelElevationSession`, `markElevationSessionLifecycleActive`, `revokeElevationSession`,
  `reconcileExpiredElevationSession` (reporting-only, unwired to any scheduler), and the pure
  expiration-validity predicate `isElevationLifecycleCapabilityValidAt`. Method names were
  deliberately chosen (and one corrected during validation) so that none could be mistaken for a
  real authorization decision. Every actor field
  (`requestedByUserId`/`granteeUserId`/`approvedByUserId`/`revokedByUserId`) is validated for
  existence and tenant membership at the service layer via a private `assertActorInTenant()` helper
  before any write, since these columns intentionally carry no foreign key.
- Two spec files: `elevation-session.service.spec.ts` (91 mocked unit tests covering every outcome
  branch of all 7 public methods plus the validity-predicate matrix) and
  `elevation-session.structural.spec.ts` (the Decision 17 proof — builds the real import graph from
  `apps/api/src` and proves zero reachability in either direction between
  `ElevationSessionService` and `RolesGuard`/`CapabilityGuard`/`JwtStrategy`/`AuthService`).
- `apps/api/test/elevation-session.e2e-spec.ts` — a real-database integration suite (10 tests, no
  HTTP, no supertest — M37 has no controller) proving: a successful transition commits both the
  state row and its audit event together; a forced audit-write failure rolls back the state
  mutation with zero partial row persisted; concurrent idempotency-key and concurrent
  non-terminal-grantee races each resolve to exactly one success at the database layer; a duplicate
  per-session permission insert and an approver-separation `CHECK` violation are both rejected
  directly by Postgres; a cross-tenant grantee fails closed; a second decide attempt on an
  already-decided item is rejected after a real round trip; and the full
  REQUESTED→APPROVED→ACTIVE→REVOKED lifecycle persists correctly end-to-end.

**Files modified (implementation, commit `0e34296`, 2026-09-05):**
- `apps/api/prisma/schema.prisma` — additive only: the two new enums, the two new models, and one
  new back-relation field (`elevationSessionCapabilities`) on the existing `Permission` model. No
  existing model's fields were altered.
- `apps/api/src/audit/audit.service.ts` — additive only: a new `logEventStrict(tx, dto)` method
  that writes from within a caller-supplied transaction and does not swallow its own errors, used
  only by `ElevationSessionService`. `logEvent()` itself is byte-for-byte unchanged (diff-verified)
  — every existing caller's write-only, best-effort, failure-swallowing behavior (AUD-1300) is
  unaffected.
- `apps/api/src/audit/enums/audit-event-type.enum.ts` — 9 new, additive `ELEVATION_SESSION_*`
  values. No existing value renamed or removed.
- `apps/api/src/identity/identity.module.ts` — registers `ElevationSessionService` as a DI provider
  only. No controller, no `@UseGuards(...)`, no `APP_GUARD`, no route.

**Architectural changes:** New `identity.elevation_sessions` / `identity.elevation_session_capabilities`
tables and two new enums exist, seeded with zero rows by default. No new module, endpoint, or route
was created. `Permission`/`RolePermission` (62 permissions, 175 mappings) are read-only referenced,
never modified.

**New integrations:** None. `ElevationSessionService` is injectable but consumed by nothing else at
runtime — no controller, guard, strategy, or interceptor calls it.

**Behavioral changes:** None observable to any existing caller. `RolesGuard`, `CapabilityGuard`'s
registration status, `JwtStrategy`, `AuthService`, every one of the 15 controller files, and every
frontend role-consuming file remain byte-for-byte unchanged — proven by the full identity suite
(11 suites, 9,553 tests) and the capability-parity suite (9,373 tests, including all 9,271
endpoint/role-subset comparisons) re-run and passing without modification.

## Validation

- **Tests added:** 91 mocked unit/structural tests (`elevation-session.service.spec.ts`,
  `elevation-session.structural.spec.ts`) plus 10 real-database integration tests
  (`elevation-session.e2e-spec.ts`) — **101/101 M37-targeted tests passed.** No existing test file
  was weakened, removed, or skipped.
- **Local validation results:** `prisma validate` — valid. `prisma generate` — succeeded.
  Repository-wide `prisma format --check` still reports a **pre-existing** formatting
  inconsistency in the `User` model, independently reproduced against the unmodified pre-M37
  schema — this is not an M37 defect and was not fixed, since doing so would be unrelated
  formatting churn inside a file M37 only additively extended. Type-check clean (0 errors); lint
  clean (0 errors/warnings, all 5 workspaces); build clean (all workspaces). Complete unit suite:
  **60 suites, 11,494 tests passed.** Complete E2E suite: **14 suites, 750 tests passed.**
  Capability-parity suite: **9,373/9,373 passed**, including all 9,271 endpoint/role-subset
  comparisons, unchanged. Migration applied cleanly on first attempt via `prisma migrate deploy`
  against the local development database; `prisma migrate status` confirmed up to date; direct SQL
  inspection confirmed exact enum values, both indexes, the partial unique index's `WHERE` clause,
  the `CHECK` constraint, and zero FK on any actor-reference column. Seed executed twice — both
  runs reported identical 62 permissions / 175 role-permission mappings / 7 roles, zero drift.
- **CI validation results:** GitHub Actions run `33995166375` (workflow `CI`, exact `head_sha`
  match to implementation commit `0e34296`) — every substantive step succeeded: Install
  dependencies, Lint, Build, Unit tests, Migrate database, Seed database, E2E tests.
- **Deterministic validation evidence:** The structural non-reachability test builds the actual
  import graph from real `import` statements across `apps/api/src` (not a hand-maintained model)
  and proves zero graph reachability between `ElevationSessionService` and every runtime
  authorization component in both directions. The real-database integration suite is the
  deterministic evidence for strict-audit transactionality and concurrency — not a mocked
  assertion that a method was called.
- **Integration validation status:** Full integration validated against a real local PostgreSQL
  instance (real `$transaction`, real unique/partial/CHECK constraints) — not unit-level mocking
  alone.
- **Known unverified areas:** True multi-process concurrency at the exact instant of
  `markElevationSessionLifecycleActive`'s defensive re-check remains theoretically possible beyond
  what a single-process test can exercise; the database-level partial unique index (Decision 13)
  is the primary enforcement mechanism for this case, with the service-layer check and P2002 catch
  as a deterministic backstop.

## Risks / Limitations

- **No runtime-effective elevation exists.** An `ACTIVE` `ElevationSession` row is a modeled data
  fact only (`GD-M37-1` Decision 20) — proven by the structural non-reachability test and by the
  unchanged capability-parity/identity suite results. No code path anywhere reads
  `ElevationSession`/`ElevationSessionCapability` to make a real authorization decision.
- **Activation remains gated on M39 plus a separate governance decision.** `GD-M37-1` Decision 21's
  three co-requirements (trustworthy step-up authentication, M39 operational, a separate explicit
  governance decision) are all still unsatisfied. This implementation does not move any of the
  three closer to being satisfied on its own.
- **No genuine MFA or step-up authentication exists.** `stepUpAuthMethod`/`stepUpVerifiedAt` remain
  nullable, are never accepted as input (M37 exposes no request surface), and are null on every row
  created by this implementation and its tests — verified directly.
- **Department, employee, candidate, resource, and Decision Case scoping**, along with any
  self-approval exception, remain explicitly deferred to **M38**, which has **not** been authorized
  or started by this entry.
- **The hire-endpoint RBAC inconsistency** (`hire.controller.ts:57` vs. `directives/10` RBAC-701)
  remains unresolved, unchanged by M37, and explicitly assigned to M40.
- **Pre-existing Prisma formatting inconsistency.** The repository-wide `prisma format --check`
  fails on the `User` model's pre-existing field alignment — independently confirmed present on
  the unmodified pre-M37 schema. This is an inherited, out-of-scope condition, not an M37 defect,
  and was deliberately left untouched rather than reformatted.

## Next Actions

1. M37 is closed. No further M37 implementation work is authorized or required.
2. **M38 — Decision Case and Approval Foundation** is the next planned milestone in the
   `GD-M35-1`/`GD-M36-1`/`GD-M37-1` roadmap. It remains **Planned** maturity and is **not
   authorized to begin** — it requires its own separate, explicit governance decision and its own
   separate implementation authorization before any code, schema, or test file for it may be
   created.
3. This PROGRESS.md entry's Status is "IMPLEMENTATION COMPLETE AND CI-CONFIRMED. CLOSED." and must
...
# M38 — Decision Case and Approval Foundation (Implementation Complete, CI-Confirmed)

> **Repository Status Classification: IMPLEMENTATION COMPLETE. CI-CONFIRMED. CLOSED.**
> This entry records the completion of `GD-M38-1` (the governance decision) and the completion of
> the M38 implementation it authorized. The Decision Case and Approval foundation now exists and
> is validated, but it is **internal, dormant, and intentionally inactive** — no controller,
> route, DTO, Swagger operation, frontend surface, or Command Center workflow exposes it; it does
> not alter existing hire, offer, recruiting, workforce, or authorization behavior; `RolesGuard`
> remains the sole runtime authorization authority; `CapabilityGuard` remains inactive; M37
> elevation sessions remain authorization-inert; the three approval tiers exist only as internal
> data/service foundations; M39's audit prerequisites (audit-read API, viewer, filtering,
> tamper-detection, retention enforcement, failed-write recovery) have not been implemented; the
> hire-endpoint inconsistency remains deferred to M40; and no runtime AI, LLM, Policy Guard,
> execution, verification, or profile cutover exists. This entry must not be read, cited, or
> summarized as evidence that any approval is or will automatically become effective, that the
> hire-endpoint RBAC inconsistency has been resolved, or that M39 has been started.

**Date:** 2026-09-05 (governance decision, `GD-M38-1`); 2026-09-05 (implementation and validation
pass); 2026-09-06 (closeout — commit, push, and CI confirmation)

**Status:** IMPLEMENTATION COMPLETE AND CI-CONFIRMED.
Implementation commit `9a6d2653d9108a3e44e1b5a59121b695e950c98a`
(`feat(decisions): implement M38 decision case foundation`). GitHub Actions CI run `34003097416`
(workflow `CI`, exact `head_sha` match) concluded **success** — Checkout, Setup Node.js, Install
dependencies, Lint, Build, Unit tests, Migrate database, Seed database, and E2E tests all passed.
`RolesGuard` remains the sole runtime authorization authority; no `DecisionCase`,
`ApprovalRequest`, or `ApprovalDecision` record has, or can have, any effect on a real
authorization decision or existing business workflow.

## Capability / Deliverable Alignment

- **Capability:** Decision Case and Approval Foundation (internal, dormant)
...
- **Current maturity classification:** **Implemented / Integrated / Tested** (per CLAUDE.md's
  Repository Maturity Classification) for the foundation layer specifically — the normalized
  seven-entity data model, migration, internal services, temporary authority-category resolver,
  strict audit-write reuse, and structural non-reachability proof exist, are unit- and
  CI-tested, and are proven to have zero effect on any real authorization decision or existing
  business workflow. **Not** "Production Ready" for approval *activation* — that remains
  explicitly unactivated and gated on M39 plus M40's hire/offer integration, neither of which this
  entry starts
- **Relevant production blueprint layers covered:** Requirements and Directives (`GD-M38-1.md`,
  including Appendix A); Execution Plan (implemented exactly as authorized); State Model (all 11
  governed enums, `DecisionCaseService`, `ApprovalService` exist and are tested against a real
  database); Test Scenarios (mocked unit suite, structural non-reachability proof, and a
  real-database integration suite covering strict-audit transactionality, three-tier bidirectional
  independence, and concurrency); System Loop (none required — no scheduler, cron, or worker
  exists or is authorized); Data Lifecycle (case creation → evidence/risk-trigger attachment →
  approval-request submission → decision recording → resolution/supersession/cancellation, all
  persisted, tenant-scoped, and versioned)
- **Relevant production blueprint layers still missing or incomplete (by design — deferred to
  future, separately governed milestones):** Failure Playbook and Environment Model for
  *activation* (no runtime consumer of `DecisionCase`/`ApprovalRequest` exists yet — M40 owns
  hire/offer integration, M39 owns the audit-read prerequisite); Evolution Strategy for automatic
  risk detection (M42), AI-prepared evidence (M44/M45), and the two-profile cutover (M46), none of
  which is started by this entry

## What Changed

**Files created (governance decision, 2026-09-05):**
- `governance/GD-M38-1.md` — see prior entry text; the governance decision and its full Appendix A
  entity reference remain exactly as originally authorized. 23 governance decisions.

**Files modified (governance decision, 2026-09-05):**
- `governance/governance_history.md`, `PROGRESS.md` header — governance-decision completion
  recorded (superseded by this entry's implementation-completion record below).

**Files created (implementation, commit `9a6d265`, 2026-09-06):**
- `apps/api/prisma/migrations/20260802000000_m38_decision_case_and_approval_foundation/migration.sql`
  — creates the new `decisions` Postgres schema, 10 new native enums plus
  `ElevationSessionScopeType` (identity schema), all 7 new tables, the tenant-scoped case
  idempotency unique index, the monotonic-version unique index, the per-requirement-category
  unique index, the at-most-one-decision-per-requirement unique index, the
  one-decider-per-request unique index, the partial unique index enforcing "at most one active
  PENDING ApprovalRequest per case" (SQL-only, following the `Offer`/`ElevationSession`
  precedent), the `DecisionCase`/`EvidenceItem` subject-nullability `CHECK` constraints, the four
  additive `ElevationSession` scope columns, their two restrictive foreign keys, and the two
  scope-consistency `CHECK` constraints. No `ON DELETE CASCADE` exists on any actor-reference or
  scope-target column — verified directly against the applied schema.
- `apps/api/src/decisions/decision-case-subject-resolver.ts` — deterministic, fail-closed
  subject/source resolver for the 5 governed `DecisionCaseSubjectType` values (Decision 6).
- `apps/api/src/decisions/authority-category-mapping.ts` — the temporary, dormant role-to-category
  qualification resolver (Hiring Manager / HR Director / Compliance Officer, plus M36-capability
  -driven `ACTION_AUTHORITY`); System Administrator does not auto-qualify for any of the three
  role-based categories (Decision 13).
- `apps/api/src/decisions/decision-case.service.ts` — `DecisionCaseService`: no HTTP interface;
  implements `createDecisionCase`, `attachEvidence`, `recordRiskTrigger`, `cancelDecisionCase`,
  `getDecisionCaseById`. Every actor field is validated for existence, tenant membership, and
  active status at the service layer before any write, since these columns intentionally carry no
  foreign key.
- `apps/api/src/decisions/approval.service.ts` — `ApprovalService`: no HTTP interface; implements
  `submitApprovalRequest` (builds the complete Tier 1/2/(3) requirement set and frozen evidence
  set for one request version, handling supersession for resubmission and risk-triggered
  escalation) and `recordApprovalDecision` (append-only decision recording, fail-fast
  GRANTED/REJECTED/NEEDS_INFORMATION resolution, bidirectional Tier 3 independence enforcement).
- `apps/api/src/decisions/decisions.module.ts` — DI registration only; no controllers array.
- Three spec files: `decision-case.service.spec.ts` (58 mocked tests), `approval.service.spec.ts`
  (50 mocked tests), and `decisions.structural.spec.ts` (the Decision 20 proof — builds the real
  import graph from `apps/api/src` and proves zero reachability in either direction between
  `DecisionCaseService`/`ApprovalService` and any controller, `RolesGuard`, `CapabilityGuard`,
  `JwtStrategy`, or `AuthService`).
- `apps/api/test/decisions.e2e-spec.ts` — a real-database integration suite (16 tests, no HTTP, no
  supertest — M38 has no controller) proving: a successful case creation commits its state and
  audit event together; a forced audit-write failure rolls back case creation with zero partial
  row persisted; a cross-tenant subject reference is rejected; the subject-nullability `CHECK`
  constraint rejects a direct bypass write; concurrent idempotency-key and concurrent
  PENDING-request-creation races each resolve to exactly one success at the database layer; full
  Tier 1, Tier 2, and risk-triggered Tier 3 lifecycles resolve correctly end-to-end, including a
  direct proof that Tier 3's independence rule works **bidirectionally** (the Tier 3 reviewer
  cannot also decide a base-tier requirement, even when otherwise qualified for it); a
  NEEDS_INFORMATION decision and subsequent resubmission correctly supersede the prior request
  version; and concurrent decisions on the same requirement resolve to exactly one surviving row.

**Files modified (implementation, commit `9a6d265`, 2026-09-06):**
- `apps/api/prisma/schema.prisma` — additive only: 7 new models, 11 new enums (10 in the new
  `decisions` schema plus `ElevationSessionScopeType` in `identity`), one new back-relation field
  each on `Permission` and `Department`, and 4 new additive fields plus 2 new relations on the
  existing `ElevationSession` model. No existing model's pre-M38 fields were altered.
- `apps/api/src/app.module.ts` — registers `DecisionsModule` for DI only. No controller import, no
  new route.
- `apps/api/src/audit/enums/audit-event-type.enum.ts` — 13 new, additive
  `DECISION_CASE_*`/`APPROVAL_*` values, reconciled exactly against `GD-M38-1` Decision 18's 11
  named operations ("each approval decision" covers 3 distinct outcome values —
  GRANTED/REJECTED/NEEDS_INFORMATION — accounting for the 11-to-13 count). No existing value
  renamed or removed; `AuditService.logEvent()` and every existing caller are byte-for-byte
  unchanged.
- `apps/api/src/identity/elevation-session.service.ts` — additive, fully backward-compatible
  scope parameters on `requestElevation()` (default `TENANT` scope, matching every pre-M38
  caller's behavior exactly) plus service-layer scope-invariant validation.
  `isElevationLifecycleCapabilityValidAt()` is byte-for-byte unchanged and still uncalled from any
  runtime path.
- `apps/api/src/identity/elevation-session.service.spec.ts` /
  `apps/api/test/elevation-session.e2e-spec.ts` — additive test cases proving backward
  compatibility and the new dormant DEPARTMENT/DECISION_CASE scope paths (including real-database
  CHECK-constraint and authorization-inertness proofs); no existing M37 test was weakened,
  skipped, or deleted.

**Architectural changes:** A new `decisions` Postgres schema with 7 tables and 10 enums now
exists, seeded with zero rows by default. `identity.elevation_sessions` gained 4 additive columns
and 2 additive restrictive foreign keys. No new module registers a controller; no new route was
created. `Permission`/`RolePermission` (62 permissions, 175 mappings) are read-only referenced,
never modified.

**New integrations:** None. `DecisionCaseService`/`ApprovalService` are injectable but consumed by
nothing else at runtime — no controller, guard, strategy, interceptor, or existing business
service (offer, hire, employee) calls them.

**Behavioral changes:** None observable to any existing caller. `RolesGuard`, `CapabilityGuard`'s
registration status, `hire.controller.ts`, `hire.service.ts`, `offer.controller.ts`,
`offer.service.ts`, every one of the 15 controller files, `ElevationSessionService`'s pre-existing
methods, and every frontend role-consuming file remain byte-for-byte unchanged — proven by the
full identity suite, the full unit and E2E suites, and the capability-parity suite re-run and
passing without modification.

## Validation

- **Tests added:** 108 mocked unit/structural tests (`decision-case.service.spec.ts`,
  `approval.service.spec.ts`, `decisions.structural.spec.ts`) plus 16 real-database integration
  tests (`decisions.e2e-spec.ts`) — **124/124 M38-targeted tests passed.** Additive M37
  scope-extension tests (unit + real-database) also pass in full. No existing test file was
  weakened, removed, or skipped.
- **Defects found and corrected during the dedicated M38 validation pass (all M38-scoped, all
  fixed prior to this closeout commit):** (1) an unrelated, independently reproduced pre-existing
  `User`-model Prisma-formatting inconsistency was reverted after each `prisma format` run rather
  than committed as unrelated churn (matching the identical observation already recorded in
  `GD-M37-1`'s own record); (2) four ESLint errors (two type-only-import corrections, one unused
  sentinel-exception class removed as dead code, one unused test import removed); (3) one
  real-database test that mis-isolated the Tier 3 bidirectional-independence assertion (the
  chosen actor failed qualification before the independence check could run) was corrected to use
  an otherwise-qualified actor, so the independence rule itself — not qualification — is what the
  test proves is enforced.
- **Local validation results:** `prisma validate` — valid. `prisma generate` — succeeded.
  Repository-wide `prisma format` still reports the same **pre-existing** `User`-model formatting
  inconsistency independently reproduced by `GD-M37-1`'s own validation — not an M38 defect, not
  corrected, since doing so would be unrelated formatting churn. Type-check clean (0 errors); lint
  clean (0 errors/warnings, all 5 workspaces); build clean (all workspaces). Complete unit suite:
  **63 suites, 11,612 tests passed.** Complete E2E suite: **15 suites, 773 tests passed.**
  Capability-parity suite passed in full within the identity suite, including all 9,271
  endpoint/role-subset comparisons, unchanged. Migration applied cleanly via `prisma migrate
  deploy` against the local development database; `prisma migrate status` confirmed up to date;
  direct SQL inspection confirmed every new schema, enum, table, column, default, index, FK,
  unique constraint, partial unique index, and CHECK constraint exactly as authored. Seed executed
  twice — both runs reported identical 62 permissions / 175 role-permission mappings / 7 roles,
  zero drift.
- **CI validation results:** GitHub Actions run `34003097416` (workflow `CI`, exact `head_sha`
  match to implementation commit `9a6d265`) — every substantive step succeeded: Install
  dependencies, Lint, Build, Unit tests, Migrate database, Seed database, E2E tests. A separate,
  later documentation-only commit and its own CI run (recorded at the top of this file) revalidate
  this same, unchanged implementation alongside the documentation update — it is not a second,
  independent implementation validation.
- **Deterministic validation evidence:** The structural non-reachability test builds the actual
  import graph from real `import` statements across `apps/api/src` (not a hand-maintained model)
  and proves zero graph reachability between `DecisionCaseService`/`ApprovalService` and every
  runtime authorization component or existing controller, in both directions. The real-database
  integration suite is the deterministic evidence for strict-audit transactionality, three-tier
  independence, and concurrency — not a mocked assertion that a method was called.
- **Audit-rollback evidence — stated precisely:** direct, real-database failure-injection was
  performed for case creation (forced `logEventStrict()` failure leaves zero partial
  `DecisionCase` rows). Every other M38 mutation family (evidence attachment, risk-trigger
  creation, request submission/supersession, decision recording/resolution, cancellation) was
  verified by direct source inspection to invoke the identical, unmodified `logEventStrict(tx,
  ...)` call inside the identical `$transaction` pattern (all 12 call sites checked, none wrapped
  in a try/catch that could swallow a thrown error before the transaction boundary), combined with
  the pre-existing, independently CI-confirmed real-database proof of this exact mechanism
  succeeding across five distinct M37 mutation types. This is combined direct-database and
  structural transactional evidence — not a separate direct failure-injection test for every M38
  mutation family individually.
- **Integration validation status:** Full integration validated against a real local PostgreSQL
  instance (real `$transaction`, real unique/partial/CHECK constraints) — not unit-level mocking
  alone.
- **Known unverified areas:** True multi-process concurrency at the exact instant of resolution
  (e.g., two decisions racing to complete the last requirement of the same request) remains
  theoretically possible beyond what a single-process test can exercise; the database-level
  unique/partial-unique indexes are the primary enforcement mechanism, with the service-layer
  checks and P2002 catches as a deterministic backstop.

## Risks / Limitations

- **No runtime-effective approval exists.** An `APPROVED` `DecisionCase` is a modeled data fact
  only (`GD-M38-1` Decision 7) — proven by the structural non-reachability test and by the
  unchanged capability-parity/identity/hire/offer suite results. No code path anywhere reads
  `DecisionCase`/`ApprovalRequest`/`ApprovalDecision` to make a real authorization decision or
  execute a business action.
- **M39's audit prerequisite remains entirely unimplemented.** `GD-M38-1`'s reuse of M37's narrow
  strict-audit-write exception is scoped to that one write path only and does not satisfy,
  replace, or partially credit any part of M39 (audit-read API, viewer, filtering,
  tamper-detection, retention enforcement, failed-write observability/recovery).
- **The hire-endpoint RBAC inconsistency** (`hire.controller.ts:59` vs. `directives/10` RBAC-701)
  remains unresolved, unchanged by M38, and explicitly assigned to M40 — M38 provides only the
  reusable approval foundation M40 will integrate.
- Automatic elevated-risk detection, deterministic Policy Guard evaluation, and any AI-prepared
  ...
  not implemented, simulated, or approximated by this implementation in any way.
- Activation of the Workforce Decision Owner or Independent Oversight Reviewer as real,
  assignable production profiles remains gated on M46, unaffected by this implementation.
- **Pre-existing Prisma formatting inconsistency.** The repository-wide `prisma format` command
  reformats the `User` model's pre-existing field alignment — independently reproduced twice
  against the unmodified pre-M38 schema and already recorded identically in `GD-M37-1`'s own
  record. This is an inherited, out-of-scope condition, not an M38 defect, and was deliberately
  left untouched rather than reformatted.

## Next Actions

1. M38 is closed. No further M38 implementation work is authorized or required.
2. **M39 — the audit-read/viewer/tamper-detection/retention prerequisite** is the next planned
   milestone in the `GD-M35-1`/`GD-M36-1`/`GD-M37-1`/`GD-M38-1` roadmap. Its governance decision
   (`GD-M39-1`) is now complete (see the dedicated M39 section below) — implementation itself
   remains **Planned** maturity and is **not authorized to begin** until a separate, explicit
   implementation instruction operating within `GD-M39-1`'s scope is given.
3. This PROGRESS.md entry's Status is "IMPLEMENTATION COMPLETE AND CI-CONFIRMED. CLOSED." and must
   not be silently upgraded to imply approval activation is in effect, that M39 has started, that
   the hire-endpoint RBAC inconsistency has been resolved, or that any later milestone (M40, M42,
   M43, M44, M45, M46) has been authorized or started.

---

# M39 — Audit Read, Recovery, and Integrity Foundation (Implementation Complete)

> **Repository Status Classification: IMPLEMENTATION COMPLETE AND CI-CONFIRMED.**
> This entry records that `GD-M39-1`'s authorized scope has been implemented, validated across two
> dedicated post-implementation validation/correction passes, committed, pushed, and confirmed by
> an exact-`head_sha` GitHub Actions CI run. It supersedes this file's own prior
> "GOVERNANCE DECISION COMPLETE. IMPLEMENTATION NOT STARTED." classification for M39. It does NOT
> record Independent Oversight Reviewer activation, authorization-effective elevation, role
> cutover, `CapabilityGuard` activation, production deployment, database-role separation, or any
> M40 or later work — none of those occurred and none is claimed here.

**Date:** 2026-09-06 (governance decision); 2026-09-06/2026-09-07 (implementation, validation, and
closeout)

**Status:** IMPLEMENTATION COMPLETE AND CI-CONFIRMED. Implementation commit
`10d352df500726fb243f8e3f6e3fcfe6617d2ca7` (`feat(audit): implement M39 audit prerequisite`).
GitHub Actions CI run `34087291647` (workflow `CI`, exact `head_sha` match) concluded **success**
across Lint, Build, Unit tests, Migrate database, Seed database, and E2E tests. Two dedicated
post-implementation validation/correction passes found and fixed real defects before this closeout
(see Validation below). M39's repository-level contribution to `GD-M35-1` Decision 9's audit
prerequisite is implemented and CI-confirmed. Activation of the Independent Oversight Reviewer
profile, any authorization-effective elevation, role cutover, or any later operating-model
capability each remains **separately governed** and requires its own future governance and
activation decision — none is authorized or claimed by this entry.

## Capability / Deliverable Alignment

- **Capability:** Audit Read, Recovery, and Integrity Foundation
- **Deliverable relevance:** Fourth implementation milestone in the `GD-M35-1` post-M35 roadmap;
  implements `GD-M35-1` Decision 9's audit prerequisite at the repository level — a tenant-scoped
  audit-read API, a permission-controlled audit viewer, filtering/search, actor/action/result/
  resource/timestamp visibility, approval-and-execution correlation, tamper-detection controls,
  retention enforcement, and observability/recovery for failed audit writes are all now real,
  running, CI-validated code
- **Current maturity classification:** **Integrated / Tested** (per CLAUDE.md's Repository
  Maturity Classification) — real code exists, is exercised by 104 M39-targeted unit tests plus 29
  M39-targeted E2E tests against a real PostgreSQL database, and is CI-confirmed; **not**
  **Production Ready** — no interactive browser walkthrough of the viewer was performed (see
  Validation below), and this milestone's own completion does not by itself activate any
  production-facing profile
- **Relevant production blueprint layers covered:** Requirements, Directives, Specs (real schema:
  3 tables, 3 enums, 6 additive `AuditEvent` fields, 9 indexes/constraints), Execution Plan (fully
  executed), State Model (`AuditWriteFailureStatus`/`AuditWriteFailureAttemptStatus`/
  `AuditChainVerificationResult` enums, governed transitions, all proven against a real database),
  Test Scenarios (unit + real-database E2E, including direct failure-injection for both new
  mutations), System Loop (automatic `@nestjs/schedule`-based recovery and chain-verification
  workers, proven to register and fire for real in non-test runtime), Failure Playbook (bounded
  recovery guarantee, append-only trigger, hash-chain tamper detection, all proven against a real
  database), Environment Model (`@nestjs/schedule` installed and generated), Data Lifecycle
  (fixed seven-year retention, no deletion/archival/purge, proven trigger-derived)
- **Relevant production blueprint layers still missing or incomplete:** Evolution Strategy — M40's
  hire/offer integration remains unimplemented and unstarted; Independent Oversight
  Reviewer/authorization-effective-elevation activation each remain a separate, future,
  unauthorized governance and activation decision; interactive browser validation of the
  `/oversight/audit` viewer was not performed (disclosed limitation, not a completed claim)

## What Changed

**Files modified (17):** `apps/api/package.json` (governed `@nestjs/schedule` dependency);
`apps/api/prisma/schema.prisma` (additive: 3 tables, 3 enums, 6 `AuditEvent` fields, 9 indexes);
`apps/api/src/app.module.ts` (`ScheduleModule.forRoot()`, audit config namespace);
`apps/api/src/audit/audit.module.ts` (registers the new controller and worker services);
`apps/api/src/audit/audit.service.ts` (+ `.spec.ts`, chain-writing path, dead-letter path,
`logOperationalEvent()`, `replayFailedWrite()`, `deriveRetentionUntil()`, and the
same-id/different-content mismatch guard); `apps/api/src/audit/enums/audit-event-type.enum.ts`
(6 additive values, 122→128); `apps/api/src/health/health.controller.ts` (+ `.spec.ts`),
`health.module.ts` (wire in the new non-gating indicator);
`apps/api/src/identity/capability-parity.spec.ts` (additive: 5 new registry entries, updated
counts, corrected shared-capability count); `apps/api/src/identity/permissions.catalog.ts` (+2
capabilities), `permissions.seed.spec.ts`, `permissions.seed.ts` (count references updated);
`apps/api/test/auth.e2e-spec.ts` (one now-permanently-failing, unguarded cleanup delete removed —
the append-only trigger makes it fail deterministically; audit_events carries no FK dependency, so
no cleanup gap resulted); `apps/web/src/components/shared/platform-header.tsx` (additive
'oversight' nav item); `package-lock.json`.

**Files created (30):** `apps/api/prisma/migrations/20260803000000_m39_audit_read_recovery_and_integrity_foundation/migration.sql`;
`apps/api/src/audit/audit-hash.util.ts` (+ `.spec.ts`), `audit-failure-classification.ts`,
`audit-recovery-worker.service.ts` (+ `.spec.ts`), `audit-chain-verification.service.ts`
(+ `.spec.ts`), `audit-events.controller.ts` (+ `.spec.ts`), `audit-events.service.ts`
(+ `.spec.ts`), `audit-response-allowlist.ts` (+ `.spec.ts`),
`dto/list-audit-events-query.dto.ts`; `apps/api/src/config/audit.config.ts`;
`apps/api/src/health/audit-integrity.health-indicator.ts` (the non-gating coarse ok/degraded
indicator); `apps/api/test/audit-events.e2e-spec.ts` (real-database E2E coverage, including
direct real-transaction failure-injection rollback proofs for both requeue and reverify, and
real-database non-gating health-degradation proofs); `apps/web/src/app/(dashboard)/oversight/audit/`
(list/detail pages, loading/error states); `apps/web/src/app/api/audit-events/{integrity/reverify,recovery/[id]/requeue}/route.ts`
(the two BFF mutation proxy routes); `apps/web/src/features/oversight/` (shell, table, filters,
recovery panel, response types).

**Architectural changes:** Additive only, within the existing `audit` Postgres schema. The
application's first new HTTP surface since M36 (5 endpoints). No existing endpoint, model, or
migration was altered or removed.

**New integrations:** `@nestjs/schedule` (in-process recurring timer; no external process, no new
infrastructure dependency) — the one governed dependency addition, powering the automatic recovery
and chain-verification workers.

**Behavioral changes:** `RolesGuard` remains the sole runtime authorization authority, applied to
the 5 new endpoints exactly as to the 73 pre-existing ones. `CapabilityGuard` remains unregistered
on every controller, global guard list, or bootstrap path. `hire.controller.ts`,
`hire.service.ts`, `offer.controller.ts`, `offer.service.ts`, and every one of the 73 pre-existing
protected endpoint mappings are byte-for-byte unchanged. `GD-M37-1`'s `ElevationSession` foundation
remains entirely authorization-inert — no file under `apps/api/src/identity/elevation-session.service.ts`
was modified. `GD-M38-1`'s `DecisionCase`/`ApprovalRequest`/`ApprovalDecision` records remain
entirely disconnected from any live workflow — no file under `apps/api/src/decisions/` was
modified. The seven-role user experience is otherwise unchanged.

## Validation

- **Tests added or updated:** 7 new unit spec files (104 tests) plus 1 rewritten spec
  (`audit.service.spec.ts`, every pre-M39 test case preserved or given a demonstrably equivalent
  replacement, verified case-by-case); `capability-parity.spec.ts` and `permissions.seed.spec.ts`
  extended additively; 1 new real-database E2E spec (`audit-events.e2e-spec.ts`, 29 tests,
  including 3 direct real-transaction failure-injection rollback proofs and 2 real-database
  non-gating health-degradation proofs); `health.controller.spec.ts` extended with a real-
  `HealthCheckService` integration block (19 tests total).
- **Verification steps performed:** Full local unit suite (69 suites/12,501 tests) and full local
  E2E suite (16 suites/802 tests) against a disposable local PostgreSQL database with the M39
  migration applied — both green. Direct real-PostgreSQL proof that the fixed seven-year retention
  derivation exactly matches PostgreSQL's own `+ INTERVAL '7 years'` arithmetic across 8 cases
  (ordinary, Feb 29 leap-day, two month-end dates, two DST-adjacent dates, sub-second, year-end) —
  an initial JavaScript implementation that instead overflowed Feb 29 into March 1 was found and
  corrected. Direct real-database proof that a persisted event's recomputed hash matches its
  stored `recordHash`. Direct real-database concurrency proof (20 parallel same-tenant writes
  produced a gap-free, duplicate-free sequence; a concurrent second tenant unaffected). Direct
  real-database proof that a same-id/different-content retry is rejected, not silently accepted (a
  new `AuditEventContentMismatchError` guard, added after this risk was identified during
  validation). Direct real-database recovery-worker and chain-verification-worker execution
  (claim, replay, stale-claim reclaim, tamper-and-detect) plus a real Nest application boot proving
  both workers register via `SchedulerRegistry` and fire from a real timer in non-test runtime.
  Direct real-transaction failure-injection proof (jest spy on the app's real DI-resolved
  `AuditService` instance; real Prisma `$transaction`; independently re-queried before/after state)
  that both requeue and reverify roll back completely on a forced strict-audit-write failure — the
  specific proof gap identified in the first validation pass's report, closed in the second.
- **Deterministic validation evidence:** Full type-check, lint, and build green across every
  workspace (api, web, config, shared, ui). `prisma validate`/`generate` clean. Migration applied
  successfully both to a populated pre-M39 database and to a fresh from-scratch database (the
  exact-SHA CI run). Seed run twice; idempotent (64 permissions/178 mappings/7 roles, stable).
- **Integration validation status:** API-level integration fully validated via real HTTP requests
  against a running NestJS application and a real PostgreSQL database. Frontend integration
  validated via type-check, lint, build (confirmed route generation for `/oversight/audit`,
  `/oversight/audit/[id]`, and both BFF routes), and direct code review of both BFF proxy routes.
  **No interactive browser walkthrough of the `/oversight/audit` viewer was performed** — this is
  a disclosed, non-blocking limitation of this implementation's validation evidence, not a
  completed browser-validation claim.
- **Known unverified areas:** Interactive browser/UI validation (see above). Everything else in
  `GD-M39-1` Decision 24's validation gate is satisfied — see `GD-M39-1.md`'s Implementation
  Completion Record for the full itemized checklist.

## Risks / Limitations

- No interactive browser walkthrough of the `/oversight/audit` viewer was performed — a known,
  disclosed, non-blocking limitation; any future reader must not treat this entry as a completed
  browser-validation claim.
- The trigger-based append-only control (`GD-M39-1` Decision 7) is explicitly bounded — reconfirmed
  live (direct `UPDATE`/`DELETE` against `audit.audit_events` both rejected) — but it does not
  protect against the database owner, a superuser, or anyone able to disable the trigger, and must
  never be described as absolute immutability by any future document.
- Database-role separation between the application runtime and migration credential remains
  explicitly out of M39's scope, recorded only as a named future security-hardening recommendation
  requiring its own, separately authorized infrastructure decision.
- The bounded failed-write-recovery guarantee (`GD-M39-1` Decision 11) explicitly does not claim
  durable recovery through a total PostgreSQL outage — this limitation is recorded, not hidden.
- Audit-integrity degradation is explicitly **non-gating** for `/health`'s HTTP status (binding
  project-owner clarification of Decision 21, recorded in `GD-M39-1.md`) — a broken chain or
  recovery backlog reports the coarse `auditIntegrity: "degraded"` signal without ever failing the
  endpoint, since restarting cannot repair either condition; operational alerting and the audit
  oversight viewer, not restart behavior, carry the signal.
- M39's completion does not by itself activate Independent Oversight Reviewer or make any
  `ElevationSession` authorization-effective — `GD-M37-1` Decision 21's three-part gate
  (trustworthy step-up authentication, M39 operational, and a separate future activation decision)
  remains additionally required; M39 satisfies only the "M39 operational" leg of that gate.
- A pre-existing, repository-wide `.catch(() => {})` convention for audit-event E2E cleanup exists
  in roughly fifteen other `*.e2e-spec.ts` files, unrelated to and not introduced by M39; only the
  one instance this implementation's own append-only trigger directly caused to fail
  deterministically (`auth.e2e-spec.ts`) was corrected, per M39's scope boundary.

## Next Actions

1. M40 — Hire and Final-Offer Maker-Checker Resolution is the next governed milestone. Its
   governance decision, `GD-M40-1`, is now complete — see the M40 section immediately below. M40
   implementation has **not** started and is **not** authorized to start except by its own
   separate, explicit implementation instruction operating within `GD-M40-1`'s authorized scope.
2. Activating Independent Oversight Reviewer as a real production profile, or making any
   `ElevationSession` authorization-effective, each requires its own separate, future governance
   and activation decision — neither is authorized by M39's completion or by `GD-M40-1`.
3. This PROGRESS.md entry's Status remains "IMPLEMENTATION COMPLETE AND CI-CONFIRMED" and must
   never be silently upgraded to imply production deployment, Independent Oversight Reviewer
   activation, authorization-effective elevation, role cutover, `CapabilityGuard` activation, or
   any M40-or-later milestone's start.

---

# M40 — Hire and Final-Offer Maker-Checker Resolution (Governance Decision)

> **Repository Status Classification: GOVERNANCE DECISION COMPLETE. IMPLEMENTATION NOT STARTED.**
> This entry records the completion of `GD-M40-1`, the governance decision authorizing a
> defined M40 implementation scope. It does NOT record any code, schema, seed, migration, test,
> configuration, dependency, or UI change, and does NOT record any change to runtime authorization
> or business-execution behavior. Current live offer and hire behavior is completely unchanged.
> M40 implementation begins only through a later, separately instructed implementation task
> operating within `GD-M40-1`'s authorized scope. This entry must not be read, cited, or
> summarized as evidence that any part of M40's implementation has occurred, that the hire-endpoint
> RBAC inconsistency has been resolved in the running system, that System Administrator elevation
> has been activated, that Independent Oversight Reviewer has been activated, or that any UI,
> capability, or audit-event change described in `GD-M40-1` exists in the running application.

**Date:** 2026-09-08

**Status:** GOVERNANCE DECISION COMPLETE. M40 implementation has NOT started. M40 remains
unimplemented, unvalidated, not CI-confirmed, and not production-ready. **Current live offer and
hire behavior is unchanged** — `hire.controller.ts` still permits only `System Administrator`/
`HR Director`; `offer.service.ts`'s `approveOffer()` is still single-actor. No application, schema,
seed, migration, test, configuration, dependency, or UI file has been created or modified. `M39`
remains complete and CI-confirmed (implementation commit
`10d352df500726fb243f8e3f6e3fcfe6617d2ca7`, CI run `34087291647`), unaffected by this entry. No
separate project-owner approval ceremony remains outstanding — `GD-M40-1` already reflects the
project owner's binding design decisions (one-packet authorization scope, System Administrator
exclusion with elevation deferred, approval/execution separation, the narrow recruiting-scoped API
boundary, capability reuse without new per-role capability names, and the post-M40/pre-M41
UI-alignment gate) supplied directly against a completed M40 implementation-readiness assessment.
The next action is a separately instructed M40 implementation task governed by `GD-M40-1`.

## Capability / Deliverable Alignment

- **Capability:** Hire and Final-Offer Maker-Checker Resolution (governance authorization only)
- **Deliverable relevance:** Fifth implementation milestone in the `GD-M35-1` post-M35 roadmap;
  resolves `GD-M35-1` Decision 13's confirmed hire-endpoint RBAC inconsistency and establishes
  `GD-M35-1` Decision 12's Tier 2 maker-checker model for routine final-offer and hire authorization
  — only once its later, separate implementation task closes; this entry records governance
  authorization only
- **Current maturity classification:** **Planned** (per CLAUDE.md's Repository Maturity
  Classification) — a governance decision exists; no Execution Plan-as-code, State Model-as-live-
  behavior, Test Scenarios, System Loop, Failure Playbook, Environment Model, Data Lifecycle, or
  Evolution Strategy layer has any implementation evidence yet
- **Relevant production blueprint layers covered:** Requirements and Directives (drafted, in
  `GD-M40-1.md`, including its Decision 28 implementation validation gate and Decision 29 non-
  binding footprint projection); Execution Plan (authorized scope defined — one-packet model,
  recruiting-scoped API, capability reuse, transaction boundaries, audit integration, minimum UI —
  zero steps executed)
- **Relevant production blueprint layers still missing or incomplete:** Specs (no new endpoint,
  DTO, or route exists; the M38 `DecisionCase`/`ApprovalRequest`/`ApprovalDecision` tables required
  no schema change but remain unconnected to any recruiting workflow), State Model (`offer.service.ts`
  and `hire.service.ts` are byte-for-byte unchanged; the Decision 15 Offer-status mapping and
  Decision 22 transaction boundaries exist in governance text only), Test Scenarios (no M40 test
  exists), System Loop (no orchestration between `offer.service.ts`/`hire.service.ts` and
  `DecisionCaseService`/`ApprovalService` exists), Failure Playbook (the elevated-risk fail-closed
  design and the strict-audit rollback design are specified in governance only), Environment Model
  (none — no new dependency is authorized), Data Lifecycle (none), Evolution Strategy (the
  unnumbered M40→M41 UI-alignment gate and M41 Search are documented as future dependencies, not
  built)

## What Changed

**Files created:**
- `governance/GD-M40-1.md` — governance decision: verified current-state facts (the exact
  single-actor hire and offer-approval RBAC, the confirmed live `Offer.status` VARCHAR values, the
  confirmed live `RolesGuard`/`CapabilityGuard` registration boundary, the confirmed M38 foundation's
  reuse-readiness); a governance-source reconciliation showing `directives/10` RBAC-701, `GD-M19-1`
  Decision 3, `GD-M35-1` Decisions 12-13, and `GD-M38-1` Decisions 19/23 as consistent, not
  conflicting; an explicit supersession of only the hire-execution portion of `GD-M19-1` Decision 3;
  a one-packet Tier 2 maker-checker authorization model (one offer-subject `DecisionCase` per final
  offer, authorizing both offer approval/issuance and later deterministic hire-conversion); two-
  authority maker-checker rules (`DEPARTMENT_OR_HIRING_AUTHORITY` = Hiring Manager,
  `HR_AUTHORITY` = HR Director, two distinct humans, reusing the existing, unmodified M38
  distinct-decider/qualification enforcement); preparation-authority rules (Recruiter, Hiring
  Manager, HR Director; System Administrator excluded); System Administrator exclusion from
  `offers:submit`/`offers:approve`/`offers:issue`/`hire:authorize` (four capabilities) with no
  emergency override and no M37 elevation activation; a 13-step offer-submission/approval/issuance/
  candidate-response/hire-execution lifecycle with approval/execution separation (three distinct
  atomic transaction boundaries); material-change invalidation rules and a no-second-approval-round
  rule; a **fully resolved, binding** Offer-status transition table (not deferred to implementation)
  re-read directly against live `offer.service.ts`, with no new status added and a documented
  semantic justification for each mapping (`NEEDS_INFORMATION` → `DRAFT`, `REJECTED` → `WITHDRAWN`,
  a first non-completing GRANTED decision leaving the offer at `PENDING_APPROVAL` unchanged); an
  elevated-risk fail-closed boundary with no automatic or manual risk-trigger creation, structurally
  reinforced by `ApprovalService`'s own fail-fast design; a narrow, recruiting-scoped API surface —
  `POST /offers/:id/approve` **retired and removed** (not repurposed, since a route named `/approve`
  would be misleading once it can also reject or request more information), replaced by three new
  routes (`GET /offers/approvals`, `GET /offers/:id/approval-case`,
  `POST /offers/:offerId/approval-requirements/:requirementId/decision`), a net −1+3 route change;
  a capability design authorizing exactly **one** new, narrowly scoped read capability
  (`offers:approvals-read`, excluding both System Administrator and Compliance Officer — correcting
  an earlier draft that would have reused the broader `offers:read`) alongside four narrow
  role-mapping changes to existing capabilities, with no per-role approval capability; an M38
  integration boundary requiring `decisions.structural.spec.ts`'s existing non-reachability proof be
  narrowed, not deleted; idempotency/concurrency rules requiring a server-derived (never
  client-supplied) `DecisionCase` idempotency key with no new schema or migration; a
  no-execution-scheduler rule; an audit-inventory-first analysis confirming exactly one new
  `AuditEventType` value remains necessary (`RECRUITING_APPROVAL_EXECUTION_BLOCKED`, projecting
  128→129), explicitly classified as **operational** (best-effort, never `logEventStrict()`, since
  a blocked attempt opens no mutating transaction) and explicitly distinguished from a failed
  execution, with every other lifecycle transition reusing existing M38/M39 events upgraded to
  `logEventStrict()`; a minimum functional recruiting-scoped approval UI (not the Independent
  Oversight area) reusing the existing GWIP shell with no new visual language; a mandatory,
  unnumbered M40→M41 UI-alignment gate referencing the project owner's newer BI Consulting
  platform, explicitly without renumbering M41-M46; a full, corrected implementation validation
  gate; a non-binding Decision 29 footprint projection with a fully reproduced before/after
  capability table (65 capabilities/179 role-permission pairs/80 protected endpoints/80 capability
  mappings/75 executable `@RequireRoles()` occurrences/127 role subsets/10,160 parity comparisons/12
  canonical role-set combinations); 30 governance decisions, corrected in a targeted follow-up pass
  on the same date

**Files modified:**
- `governance/governance_history.md` — added the "M40 — Hire and Final-Offer Maker-Checker
  Resolution" section indexing `GD-M40-1`, following the established chronological index format; no
  existing entry was duplicated or edited
- `PROGRESS.md` — this entry, plus updated header lines (Last Updated, Updated By, Previous Update,
  Current Phase, Overall Classification, Active Sprint) and one accuracy correction to the M39
  section's own forward-looking "Next Actions" line about M40's governance status, recording
  governance-decision completion
...
`hire.controller.ts`, `hire.service.ts`, `offer.controller.ts`, `offer.service.ts`,
`authority-category-mapping.ts`, `decision-case-subject-resolver.ts`, `decision-case.service.ts`,
`approval.service.ts`, `decisions.module.ts`, `decisions.structural.spec.ts`,
`permissions.catalog.ts`, `audit-event-type.enum.ts`, `audit-response-allowlist.ts`, and every
frontend recruiting/oversight file remain byte-for-byte unchanged.

## Validation

- **Tests added or updated:** None. No test file was created, modified, or deleted.
- **Verification steps performed:** A dedicated M40 implementation-readiness assessment inspecting
  the running codebase directly — full reads of `hire.controller.ts`, `hire.service.ts`,
  `offer.controller.ts`, `offer.service.ts`, every M38 entity/service/mapping/resolver file, the
  M38 migration SQL's unique indexes and CHECK constraints, `permissions.catalog.ts`'s live
  capability keys and role mappings, and direct confirmation of `Offer.status`'s live VARCHAR
  values and `OFFER_TERMINAL_STATUSES`; background research confirming `RolesGuard`/
  `CapabilityGuard`/elevation-inert registration status with file:line citations, the full frontend
  recruiting/oversight UI inventory, and the exact existing hire/offer test-coverage inventory
  (RBAC-metadata tests, single-actor E2E groups, `capability-parity.spec.ts`'s exact
  `ENDPOINT_REGISTRY`/count assertions); direct reading of `GD-M35-1`, the M35 blueprint document,
  the relevant `GD-M18-1`/`GD-M19-1`/`GD-M38-1` decisions, and `directives/10` RBAC-700/701; the
  project owner's binding design corrections applied directly against the assessment's draft
  recommendations before this decision was drafted.
- **Deterministic validation evidence:** Not applicable — no code exists to validate
  deterministically. Type-check, lint, and test suite runs were not performed and are not
  applicable to this governance-only milestone step.
- **Integration validation status:** Not applicable — no integration exists.
- **Known unverified areas:** None within M40 governance's own scope. The eventual implementation
  task's full validation gate (`GD-M40-1` Decision 28) is specified but not yet exercised — that is
  implementation-time work, not governance work. `GD-M40-1` Decision 15's Offer-status transition
  table is fully resolved by governance itself (re-read directly against live `offer.service.ts` as
  part of this correction pass) and is not deferred — implementation may reconfirm exact file/line
  locations only, not redesign the behavior.

## Risks / Limitations

- This entry and `GD-M40-1.md` describe an **authorized scope**, not running behavior. Any future
  reader (human or agent) must not treat `GD-M40-1` as having implemented the recruiting-scoped
  approval API, the retirement of `POST /offers/:id/approve`, the minimum approval UI, or any
  capability/role-mapping change, and must not read this decision as having resolved `GD-M35-1`
  Decision 13's hire-endpoint RBAC inconsistency in the running system — only a later, separate
  implementation and validation pass can do that.
- `GD-M40-1` Decision 29's footprint figures (129 `AuditEventType` values, 65 capabilities, 179
  role-permission pairs, 80 protected endpoints, 12 canonical role-set combinations, and related
  counts) are a **non-binding projection**, computed reproducibly from Decision 17's route table and
  Decision 18's capability table as of 2026-09-08 (corrected in this pass from an earlier
  arithmetic error that undercounted System Administrator removals), and must be re-verified — not
  assumed — against the exact final route and capability inventory at implementation time; the
  underlying DESIGN these figures derive from is fully governed and final, not open for
  implementation-time redesign.
- System-Administrator-via-elevation substitution remains explicitly deferred — `GD-M40-1`
  Decision 9 authorizes no emergency override and no M37 elevation activation; any future
  substitution requires its own separate governance decision, made only after authorization-
  effective elevation is explicitly approved.
- The post-M40/pre-M41 UI-alignment gate (`GD-M40-1` Decision 25) is itself unauthorized as an
  implementation step — it is a recorded, mandatory future governance checkpoint, not yet
  scheduled, resourced, or drafted as its own decision.

## Next Actions

1. A separate, explicit implementation instruction is required before any M40 implementation work
   begins. That instruction must operate within `GD-M40-1`'s authorized scope (the one-packet
   authorization model, the recruiting-scoped API design, the capability-reuse design, the
   transaction/idempotency/concurrency rules, the audit-integration design, the minimum UI, and the
   Decision 28 validation gate) and must not diverge from it without its own governance amendment.
2. When that instruction is given, implementation proceeds per `GD-M40-1`'s sequence: reconfirm
   Decision 15's already-governed Offer-status transition table against live code (file/line
   locations only, not redesign) → narrow `decisions.structural.spec.ts`'s non-reachability proof
   (Decision 19) → wire the recruiting-scoped orchestration (Decision 10-13) → remove
   `POST /offers/:id/approve` and add the three new routes (Decision 17) → the four capability
   role-mapping changes plus the one new `offers:approvals-read` capability (Decision 18) → the
   strict-audit upgrade and the one new, operationally-classified `AuditEventType` value (Decision
   23) → the minimum functional UI (Decision 24) → the full Decision 28 validation gate →
   PROGRESS.md update recording real implementation and validation evidence → commit → push → CI.
3. The unnumbered M40→M41 UI-alignment gate (`GD-M40-1` Decision 25) is a separate, future,
   unauthorized governance step required after M40 implementation closes and before M41 begins.
4. This PROGRESS.md entry's Status remains "GOVERNANCE DECISION COMPLETE. IMPLEMENTATION NOT
   STARTED." until a future, separate entry exists — this entry must never be silently upgraded to
   imply implementation, validation, CI confirmation, production readiness, hire-endpoint RBAC
   resolution, System Administrator elevation activation, or Independent Oversight Reviewer
   activation occurred.
